...
async function testOfflineMode() {
  ...
  try {
    ...
    for (const result of results) {
      ...
      console.log(
        `   ${status} ${result.service.toUpperCase()}: ${result.valid ? 'Valid' : result.error}`
      )
    }
    ...
    const openaiResponse = await fetch('http://localhost:3001/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'user', content: 'Evaluate this business idea: AI-powered code review' },
        ],
        max_tokens: 100,
      }),
    })
    ...
    console.log(
      `      Content preview: ${openaiData.choices[0].message.content.substring(0, 100)}...`
    )
    ...
    const anthropicResponse = await fetch('http://localhost:3002/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 100,
        messages: [
          { role: 'user', content: 'Analyze this market opportunity: sustainable packaging' },
        ],
      }),
    })
