...
export const useWebSocketStore = defineStore('websocket', () => {
    ...
    try {
      ...
      if (connected) {
        ...
        addNotification({
          type: 'success',
          title: 'Connected',
          message: webSocket.usingSse.value ? 'Connected via Server-Sent Events' : 'Connected via WebSocket',
          autoClose: true
        });
      } else {
  ...
  const handleError = (event: ErrorEvent) => {
    ...
    const notificationType = event.severity === 'high' || event.severity === 'critical' ? 'error' : 'warning';
