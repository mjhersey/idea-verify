...
describe('MultiAgentQueueManager', () => {
  ...
  describe('Error Handling', () => {
    test('should handle queue initialization failures', async () => {
      ...
      vi.mocked(QueueFactory.createQueue).mockRejectedValueOnce(new Error('Redis connection failed'));
  ...
  describe('Shutdown', () => {
    test('should shutdown all queues', async () => {
      const closeSpies = Array.from(mockQueues.values()).map(queue => 
        vi.spyOn(queue, 'close')
      );
