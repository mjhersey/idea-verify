...
describe('PainPointAnalysisEngine', () => {
  ...
  describe('analyzePainPoints', () => {
    ...
    it('should analyze business domain pain points correctly', async () => {
      ...
      const hasBusinessPain = result.painPoints.some(pain => 
    ...
    it('should handle technology domain pain points', async () => {
      ...
      const hasTechPain = result.painPoints.some(pain => 
      ...
      const avgOpportunity = result.painPoints.reduce((sum, p) => sum + p.opportunity, 0) / result.painPoints.length;
    ...
    it('should identify critical pain points correctly', async () => {
      ...
      const criticalPains = result.painPoints.filter(p => 
        p.severity === 'high' && (p.frequency === 'frequent' || p.frequency === 'constant')
      );
    ...
    it('should calculate opportunity scores appropriately', async () => {
      ...
      const highSeverityFrequentPains = result.painPoints.filter(p => 
        p.severity === 'high' && (p.frequency === 'frequent' || p.frequency === 'constant')
      );
      ...
      const avgOpportunity = result.painPoints.reduce((sum, p) => sum + p.opportunity, 0) / result.painPoints.length;
      expect(Math.abs(result.painPointMetrics.overallOpportunityScore - avgOpportunity)).toBeLessThan(5);
    });
