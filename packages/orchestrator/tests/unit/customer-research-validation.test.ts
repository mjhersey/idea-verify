...
describe('ProblemValidationEngine', () => {
  ...
  describe('validateProblem', () => {
    it('should successfully validate problem for health & fitness domain', async () => {
      ...
      expect(['rare', 'occasional', 'frequent', 'constant']).toContain(
        result.problemValidation.frequency
      )
    ...
    it('should adjust metrics based on target market scope', async () => {
      ...
      expect(globalResult.marketDemand.demandIntensity).toBeGreaterThan(
        localResult.marketDemand.demandIntensity
      )
    })
