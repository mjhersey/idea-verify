...
describe('WillingnessToPayEngine', () => {
  ...
  describe('analyzeWillingnessToPay', () => {
    it('should successfully analyze willingness to pay for health & fitness domain', async () => {
      ...
      expect(result.willingnessToPay.priceRange.max).toBeGreaterThan(
        result.willingnessToPay.priceRange.min
      )
      ...
      expect(['subscription', 'one-time', 'freemium', 'usage-based', 'hybrid']).toContain(
        result.willingnessToPay.priceModel
      )
    ...
    it('should generate comprehensive pricing insights', async () => {
      ...
      expect(result.pricingInsights.optimalPriceRange.max).toBeGreaterThan(
        result.pricingInsights.optimalPriceRange.min
      )
    ...
    it('should provide actionable pricing recommendations', async () => {
      ...
      expect(['subscription', 'one-time', 'freemium', 'usage-based', 'hybrid']).toContain(
        result.pricingRecommendations.pricingModel
      )
    ...
    it('should handle technology domain pricing appropriately', async () => {
      ...
      const hasTechFeatures = tierFeatures.some(
        feature =>
    ...
    it('should analyze price sensitivity correctly', async () => {
      ...
      expect(['high', 'medium', 'low']).toContain(
        result.willingnessToPay.priceSensitivity.elasticity
      )
    ...
    it('should assess affordability comprehensively', async () => {
      ...
      expect(result.willingnessToPay.affordabilityAssessment.paymentPreferences).toBeInstanceOf(
        Array
      )
      expect(
        result.willingnessToPay.affordabilityAssessment.paymentPreferences.length
      ).toBeGreaterThan(0)

      expect(result.willingnessToPay.affordabilityAssessment.budgetConstraints).toBeInstanceOf(
        Array
      )
      expect(
        result.willingnessToPay.affordabilityAssessment.budgetConstraints.length
      ).toBeGreaterThan(0)
    })
    ...
    it('should generate demand curve with realistic data points', async () => {
      ...
      for (let i = 1; i < demandCurve.length; i++) {
        ...
        expect(demandCurve[i].demandPercentage).toBeLessThanOrEqual(
          demandCurve[i - 1].demandPercentage
        )
      }
    ...
    it('should handle pricing for different target markets', async () => {
      ...
      expect(globalResult.willingnessToPay.priceRange.max).toBeGreaterThanOrEqual(
        localResult.willingnessToPay.priceRange.max
      )
    })
