  ...
  beforeEach(() => {
    ...
    mockRequest = {
      businessIdea: {
        ...
        description: 'An AI-driven platform that personalizes learning experiences for students across different subjects and skill levels'
      },
  ...
  describe('Enhanced Capabilities', () => {
    ...
    it('should execute comprehensive TAM/SAM/SOM analysis', async () => {
      ...
      expect(result.insights.some(insight => 
        insight.includes('Total Addressable Market (TAM)')
      )).toBe(true);
      
      expect(result.insights.some(insight => 
        insight.includes('Serviceable Addressable Market (SAM)')
      )).toBe(true);
      
      expect(result.insights.some(insight => 
        insight.includes('Serviceable Obtainable Market (SOM)')
      )).toBe(true);
      
      // Verify methodology information
      expect(result.insights.some(insight => 
        insight.includes('top-down') || insight.includes('bottom-up') || insight.includes('value-theory')
      )).toBe(true);
    });
    ...
    it('should provide opportunity scoring (0-100 scale)', async () => {
      ...
      expect(result.insights.some(insight => 
        insight.includes('Market opportunity score:') && insight.includes('/100')
      )).toBe(true);
      
      // Verify component scores are mentioned
      expect(result.insights.some(insight => 
        insight.includes('Strongest factor:') || insight.includes('Key challenge:')
      )).toBe(true);
    });
    ...
    it('should include confidence assessment', async () => {
      ...
      expect(result.insights.some(insight => 
        insight.includes('Analysis confidence:') && insight.includes('data quality')
      )).toBe(true);
    });
    ...
    it('should provide growth rate analysis', async () => {
      ...
      expect(result.insights.some(insight => 
        insight.includes('Market growing at') && insight.includes('% annually')
      )).toBe(true);
    });
    ...
    it('should include competitive landscape analysis', async () => {
      ...
      expect(result.insights.some(insight => 
        insight.includes('competitors with') && insight.includes('structure')
      )).toBe(true);
      
      expect(result.insights.some(insight => 
        insight.includes('Entry barriers are')
      )).toBe(true);
    });
