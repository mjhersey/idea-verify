...
describe('MarketResearchAgent', () => {
  ...
  describe('successful execution', () => {
    it('should execute successfully with comprehensive market data', async () => {
      ...
      expect(result.insights.some(insight => insight.includes('$50.0B'))).toBe(true)

      expect(result.insights.some(insight => insight.includes('2 key competitors'))).toBe(true)
  ...
  describe('error handling', () => {
    it('should handle LLM provider errors gracefully', async () => {
      mockAnalyzeMarketResearch.mockRejectedValue(new Error('LLM service unavailable'))
  ...
  describe('insights extraction', () => {
    it('should extract currency-formatted market size insights', async () => {
      ...
      expect(result.insights.some(insight => insight.includes('$1.5B'))).toBe(true)

      expect(result.insights.some(insight => insight.includes('$150.0M'))).toBe(true)

      expect(result.insights.some(insight => insight.includes('rapidly growing'))).toBe(true)
    })
    ...
    it('should extract competitor insights', async () => {
      ...
      expect(result.insights.some(insight => insight.includes('3 key competitors'))).toBe(true)
    ...
    it('should extract trend and opportunity insights', async () => {
      ...
      expect(result.insights.some(insight => insight.includes('1 high-impact trends'))).toBe(true)

      expect(
        result.insights.some(insight => insight.includes('Market trend: AI automation trend'))
      ).toBe(true)

      expect(
        result.insights.some(insight => insight.includes('Opportunity: Underserved market segment'))
      ).toBe(true)
    })
  })
