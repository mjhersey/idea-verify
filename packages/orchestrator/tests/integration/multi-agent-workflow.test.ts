...
describe('Multi-Agent Workflow Integration Tests', () => {
  ...
  const testAgents = {
    ...
    'competitive-analysis': new IntegrationTestAgent(
    ...
    'financial-analysis': new IntegrationTestAgent(
      'financial-analysis',
  ...
  describe('Single Agent Workflow', () => {
    ...
    test('should handle agent execution timeout', async () => {
      ...
      await expect(
        orchestrator.executeWorkflow('workflow-002', 'eval-002', businessIdea, {
          requiredAgents: ['market-research'],
  ...
  describe('Multi-Agent Parallel Workflow', () => {
    ...
    test('should handle partial agent failures gracefully', async () => {
      ...
      const unreliableAgents = {
        ...
        'customer-research': new IntegrationTestAgent(
          'customer-research',
          ['market-research'],
          1000,
          0
  ...
  describe('Dependency Resolution', () => {
    test('should validate dependencies before execution', async () => {
      ...
      await expect(
        orchestrator.executeWorkflow('workflow-006', 'eval-006', businessIdea, {
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle circuit breaker activation', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        attempts.push(
          orchestrator
            .executeWorkflow(`workflow-008-${i}`, `eval-008-${i}`, businessIdea, {
              requiredAgents: ['market-research'],
            })
            .catch(error => error)
        )
    ...
    test('should execute compensation actions on failures', async () => {
      ...
      vi.spyOn(authFailingAgent, 'execute').mockRejectedValue(
      ...
      try {
        await orchestrator.executeWorkflow('workflow-009', 'eval-009', businessIdea, {
          requiredAgents: ['market-research'],
        })
      } catch (error) {
        // Expected to fail
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Refreshing credentials'))
  ...
  describe('Workflow State Management', () => {
    ...
    test('should maintain workflow statistics', async () => {
      ...
      await orchestrator.executeWorkflow('workflow-014', 'eval-014', businessIdea, {
        requiredAgents: ['market-research'],
      })
