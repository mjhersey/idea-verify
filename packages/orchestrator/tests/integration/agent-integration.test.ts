...
describe('AgentService Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockRequest = {
      businessIdea: {
        ...
        description: 'A mobile app that uses AI to provide personalized fitness recommendations and workout plans'
      },
  ...
  describe('Market Research Agent', () => {
    ...
    it('should handle different business idea types', async () => {
      const techRequest = {
        ...
        businessIdea: {
          ...
          description: 'A decentralized platform for tracking products through the supply chain using blockchain technology'
        },
    ...
    it('should return consistent results for the same input', async () => {
      const result1 = await agentService.executeAgent(
        'market-research',
        mockRequest,
        mockContext
      );

      const result2 = await agentService.executeAgent(
        'market-research',
        mockRequest,
        { ...mockContext, evaluationId: 'eval-124' }
      );
