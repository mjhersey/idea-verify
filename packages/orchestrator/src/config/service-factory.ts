...
export class ServiceFactory {
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      if (
        this.services.databaseManager &&
        typeof this.services.databaseManager.shutdown === 'function'
      ) {
  ...
  async healthCheck(): Promise<{
    ...
    try {
      ...
      if (
        ...
        typeof this.services.queueManager.healthCheck === 'function'
      ) {
      ...
      if (
        this.services.databaseManager &&
        typeof this.services.databaseManager.healthCheck === 'function'
      ) {
  ...
  static async createDatabaseManager(useMock: boolean = false): Promise<any> {
    if (useMock) {
      const mockDb = MockServiceFactory.createDatabaseManager(
        ServiceFactory.getDevelopmentConfig().database
      )
