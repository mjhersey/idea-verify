...
export class LandscapeVisualizationEngine {
  ...
  async generateVisualizationData(
    ...
    console.log(
      `[LandscapeVisualizationEngine] Generating visualization data for ${competitors.length} competitors`
    )

    try {
      // Generate all visualization components in parallel
      const [competitorMatrix, marketMap, featureComparison, pricingLandscape] = await Promise.all([
      ...
      console.log(
        `[LandscapeVisualizationEngine] Visualization data generated in ${Date.now() - startTime}ms`
      )
  ...
  private async createMarketMap(
    ...
    const segments = Array.from(segmentMap.entries()).map(([segmentName, segmentCompetitors]) => {
      ...
      const avgFeatureCount =
        segmentCompetitors.reduce((sum, c) => sum + c.features.length, 0) /
        segmentCompetitors.length
  ...
  private async createFeatureComparisonMatrix(
    competitors: CompetitorProfile[]
    ...
    const gaps = features
      .map(feature => {
        ...
        const opportunity = Math.max(
          0,
          100 - (availableCount / competitors.length) * 80 - avgQuality * 0.2
        )
  ...
  private identifyOpportunityAreas(
    ...
    for (let x = 0; x < 100; x += gridSize) {
      for (let y = 0; y < 100; y += gridSize) {
        const nearbyCompetitors = positionedCompetitors.filter(
          comp =>
  ...
  private identifySegmentConnections(
    ...
    for (let i = 0; i < segments.length; i++) {
      for (let j = i + 1; j < segments.length; j++) {
        ...
        const commonCompetitors = segment1.competitors.filter(c => segment2.competitors.includes(c))
        ...
        if (commonCompetitors.length > 0) {
          connections.push({
            ...
            to: segment2.name,
            strength:
              (commonCompetitors.length /
                Math.min(segment1.competitors.length, segment2.competitors.length)) *
              100,
            type: 'substitution',
          })
        }
