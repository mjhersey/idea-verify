...
export interface PricingInformation {
  model: 'subscription' | 'one-time' | 'freemium' | 'transaction-based' | 'usage-based' | 'hybrid' | 'unknown';
  tiers: PricingTier[];
  currency: string;
  lastUpdated: Date;
  source: string;
...
export interface DataSource {
  name: string;
  url?: string;
  type: 'company-website' | 'review-site' | 'industry-report' | 'news' | 'social-media' | 'search-engine';
