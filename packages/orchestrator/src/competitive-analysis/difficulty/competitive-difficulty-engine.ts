...
export class CompetitiveDifficultyEngine {
  ...
  async analyzeDifficulty(
    ...
    console.log(
      `[CompetitiveDifficultyEngine] Analyzing market difficulty for ${competitors.length} competitors`
    )
    ...
    try {
      ...
      const components = await this.calculateDifficultyComponents(
        competitors,
        barriers,
        businessIdea
      )
      ...
      console.log(
        `[CompetitiveDifficultyEngine] Difficulty analysis completed in ${Date.now() - startTime}ms`
      )
      ...
      throw new Error(
        `Difficulty analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private async analyzeBarriersToEntry(
    ...
    const overall: BarrierToEntry['overall'] =
      averageScore > 70 ? 'high' : averageScore > 40 ? 'medium' : 'low'
  ...
  private analyzeFinancialBarriers(
    ...
    if (ideaText.includes('enterprise') || ideaText.includes('b2b')) {
      score += 20
      description +=
        '. Enterprise sales require substantial investment in sales teams and long sales cycles'
    ...
    if (highThreatCompetitors > 2) {
      score += 15
      description +=
        '. Strong existing competitors require higher marketing spend to gain market share'
    }
  ...
  private analyzeRegulatoryBarriers(businessIdea: {
    title: string
    description: string
    targetMarket?: string
    category?: string
    ...
    if (
      ideaText.includes('fintech') ||
      ideaText.includes('financial') ||
      ideaText.includes('payment')
    ) {
      score += 40
      requirements.push(
    ...
    return {
      ...
      requirements:
        requirements.length > 0
          ? requirements
          : ['Standard business registration', 'Tax compliance', 'General data protection'],
    }
  }
  ...
  private analyzeTechnologicalBarriers(
    ...
    if (ideaText.includes('platform') || ideaText.includes('marketplace')) {
      ...
      complexityFactors.push(
        ...
        'Multi-sided platform complexity',
    ...
    if (
      ideaText.includes('real-time') ||
      ideaText.includes('trading') ||
      ideaText.includes('gaming')
    ) {
      score += 20
      complexityFactors.push(
    ...
    return {
      ...
      complexityFactors:
        complexityFactors.length > 0
          ? complexityFactors
          : ['Standard web development', 'Database management', 'Basic security implementation'],
    }
  }
  ...
  private analyzeBrandBarriers(competitors: CompetitorProfile[]): BarrierToEntry['brand'] {
    ...
    const strongBrandCompetitors = competitors.filter(
      c => (c.marketShare && c.marketShare > 15) || c.threatLevel === 'high'
    )
    ...
    return {
      ...
      description,
      establishedBrands:
        establishedBrands.length > 0
          ? establishedBrands
          : ['Market incumbents with established presence'],
    }
  }
  ...
  private analyzeDistributionBarriers(
    ...
    return {
      ...
      description,
      channels:
        channels.length > 0 ? channels : ['Online marketing', 'Direct sales', 'Digital channels'],
    }
  }
  ...
  private analyzeNetworkBarriers(
    ...
    return {
      ...
      description,
      networkEffects:
        networkEffects.length > 0
          ? networkEffects
          : ['Standard customer retention', 'Basic user engagement'],
    }
  }
  ...
  private async calculateDifficultyComponents(
    ...
    const barrierHeight =
      (barriers.financial.score +
        barriers.regulatory.score +
        barriers.technological.score +
        barriers.brand.score +
        barriers.distribution.score +
        barriers.network.score) /
      6
  ...
  private generateDifficultyAnalysis(
    ...
    if (overall >= 75) {
      explanation =
        'Very high competitive difficulty. Market entry will be extremely challenging due to strong competitors, high barriers, and saturated conditions.'
      riskFactors.push(
        'Extremely well-funded competitors',
      ...
      explanation =
        'High competitive difficulty. Success will require significant resources, strong differentiation, and strategic execution.'
      ...
      explanation =
        'Moderate competitive difficulty. Market entry is feasible with proper planning and execution, though challenges exist.'
      ...
      explanation =
        'Low competitive difficulty. Market conditions are favorable for new entrants with lower barriers to entry.'
    ...
    if (barriers.regulatory.score > 70) {
      ...
      mitigationStrategies.push(
        'Build regulatory expertise early and factor compliance into timeline'
      )
    }
  ...
  private calculateCompetitorStrength(competitors: CompetitorProfile[]): number {
    ...
    const avgMarketShare =
      competitors.reduce((sum, c) => sum + (c.marketShare || 5), 0) / competitors.length
