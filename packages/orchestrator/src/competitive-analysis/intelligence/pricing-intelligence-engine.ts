...
export class PricingIntelligenceEngine {
  ...
  async analyzePricing(
    ...
    console.log(
      `[PricingIntelligenceEngine] Analyzing pricing for ${competitors.length} competitors`
    )
    ...
    try {
      ...
      console.log(
        `[PricingIntelligenceEngine] Pricing analysis completed in ${Date.now() - startTime}ms`
      )
      ...
      throw new Error(
        `Pricing analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }

  private async enhancePricingData(
    competitors: CompetitorProfile[]
  ): Promise<Map<string, PricingInformation>> {
  ...
  private async enhanceCompetitorPricing(
    competitor: CompetitorProfile
  ): Promise<PricingInformation> {
  ...
  private async researchPricingTiers(competitor: CompetitorProfile): Promise<PricingTier[]> {
    ...
    if (
      competitor.positioning.pricePositioning === 'freemium' &&
      !baseTiers.some(t => t.price === 0)
    ) {
    ...
    if (
      competitor.positioning.marketSegment === 'enterprise' &&
      !baseTiers.some(t => t.price > 100)
    ) {
      enhancedTiers.push({
        name: 'Enterprise',
    ...
    for (let i = 0; i < prices.length - 1; i++) {
      if (prices[i + 1] / prices[i] > 4 && prices[i] > 0) {
        // Large gap
  ...
  private refinePricingModel(
    ...
    if (competitor.positioning.valueProposition.toLowerCase().includes('usage'))
      return 'usage-based'
    if (competitor.positioning.valueProposition.toLowerCase().includes('transaction'))
  ...
  private analyzePricingModels(
    pricingMap: Map<string, PricingInformation>
  ): PricingLandscapeData['pricingModels'] {
  ...
  private identifyPricingOpportunities(
    ...
    if (
      businessIdea.description.toLowerCase().includes('ai') ||
      businessIdea.description.toLowerCase().includes('automation')
    ) {
      opportunities.push({
  ...
  private async generatePricingInsights(
    ...
    if (dominantModel.marketShare > 60) {
      insights.push({
        ...
        confidence: 80,
        supportingData: [
          `${dominantModel.competitors.length} of ${pricingMap.size} competitors use ${dominantModel.model}`,
        ],
      })
    }
    ...
    // Tier analysis
    const avgTiers =
      Array.from(pricingMap.values()).reduce((sum, p) => sum + p.tiers.length, 0) / pricingMap.size
    if (avgTiers < 2) {
      insights.push({
        type: 'tier-missing',
  ...
  private recommendPricingModel(
    ...
    if (
      ...
      description.includes('platform') ||
      targetMarket.includes('business')
    ) {
      return 'subscription'
    }
  ...
  private generatePricingRationale(
    ...
    if (modelShare < 30) {
      rationale.push(
        `${model} model is underutilized in market (${modelShare.toFixed(1)}% adoption)`
      )
    }
  ...
  private calculatePricingConfidence(pricingMap: Map<string, PricingInformation>): number {
    ...
    const avgConfidence =
      Array.from(pricingMap.values()).reduce((sum, p) => sum + p.confidence, 0) / pricingMap.size

    // Boost confidence based on data richness
    const avgTiersPerCompetitor =
      Array.from(pricingMap.values()).reduce((sum, p) => sum + p.tiers.length, 0) / pricingMap.size
  ...
  private calculateDataFreshness(pricingMap: Map<string, PricingInformation>): number {
    ...
    const now = new Date()
    const avgAge =
      Array.from(pricingMap.values()).reduce((sum, p) => {
