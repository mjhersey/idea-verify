...
import {
  MultiAgentQueueManager,
  MultiAgentJobData,
  AgentExecutionJob,
} from '../queue/multi-agent-queue-manager.js'
...
export class MultiAgentOrchestrator extends EventEmitter {
  ...
  private setupDefaultWorkflows(): void {
    ...
    const fastWorkflow: WorkflowDefinition = {
      ...
      parallelGroups: [['market-research'], ['financial-analysis']],
      executionOrder: [0, 1],
  ...
  async executeWorkflow(
    ...
    const effectiveWorkflow: WorkflowDefinition = {
      ...
      dependencies: Object.fromEntries(
        Object.entries(workflow.dependencies)
          ...
          .map(([agent, deps]) => [agent, deps.filter(dep => effectiveAgents.includes(dep))])
      ),
      // Get parallel execution groups from agent registry if available, otherwise calculate
      parallelGroups:
        this.agentRegistry.getParallelExecutionGroups().length > 0
          ? this.agentRegistry
              .getParallelExecutionGroups()
              .filter(group => group.some(agent => effectiveAgents.includes(agent)))
    ...
    console.log(
      `[MultiAgentOrchestrator] Started workflow: ${workflowId} for evaluation: ${evaluationId}`
    )
  ...
  private async handleAgentExecutionCompleted(event: {
    ...
    this.updateWorkflowProgress(execution)
    console.log(
      `[MultiAgentOrchestrator] Agent ${event.agentType} completed for evaluation: ${execution.evaluationId}`
    )
  }
  ...
  private async handleAgentExecutionFailed(event: {
    ...
    if (agentStatus.retryCount < workflow.retryPolicy.maxRetries) {
      ...
      const delay = this.calculateRetryDelay(workflow.retryPolicy, agentStatus.retryCount)

      console.log(
        `[MultiAgentOrchestrator] Retrying ${event.agentType} in ${delay}ms (attempt ${agentStatus.retryCount})`
      )
  ...
  private async checkWorkflowCompletion(execution: WorkflowExecution): Promise<void> {
    const allCompleted = Array.from(execution.agentStatuses.values()).every(
      status =>
        status.status === 'completed' || status.status === 'failed' || status.status === 'skipped'
    )
  ...
  private async completeWorkflow(execution: WorkflowExecution): Promise<void> {
    ...
    console.log(
      `[MultiAgentOrchestrator] Workflow completed: ${execution.workflowId} for evaluation: ${execution.evaluationId}`
    )
    this.emit('workflowCompleted', { execution, results })
  }

  private async retryAgentExecution(
    execution: WorkflowExecution,
    agentType: AgentType
  ): Promise<void> {
  ...
  private async handleAgentFailure(
    ...
    if (dependentAgents.length === 0) {
      ...
      console.log(
        `[MultiAgentOrchestrator] Non-critical agent ${failedAgentType} failed, continuing workflow`
      )
      return
    }
    ...
    for (const dependentAgent of dependentAgents) {
      ...
      if (dependentStatus && dependentStatus.status === 'pending') {
        ...
        console.log(
          `[MultiAgentOrchestrator] Skipping ${dependentAgent} due to failed dependency: ${failedAgentType}`
        )
      }
    }

    // Check if workflow can still produce meaningful results
    const completedAgents = Array.from(execution.agentStatuses.values()).filter(
  ...
  private estimateGroupDuration(agents: AgentType[]): number {
    ...
    const maxDuration = Math.max(...agents.map(agent => baseDurations[agent] || 120000))
  ...
  private estimateMemoryRequirement(agents: AgentType[]): number {
    ...
    return agents.reduce((total, agent) => total + (baseMemory[agent] || 256), 0)
  }
  ...
  private assessComputeIntensity(agents: AgentType[]): 'low' | 'medium' | 'high' {
    ...
    const avgIntensity =
      agents.reduce((total, agent) => total + (intensityScores[agent] || 2), 0) / agents.length
  ...
  buildDependencyGraph(agentTypes: AgentType[]): {
    nodes: AgentType[]
  ...
  getStatistics(): {
    ...
    const completedWorkflows = Array.from(this.executionHistory.values()).filter(
      execution => execution.status === 'completed'
    ).length
    const failedWorkflows = Array.from(this.executionHistory.values()).filter(
      execution => execution.status === 'failed'
    ).length

    // Calculate average execution time from completed workflows
    const completedExecutions = Array.from(this.executionHistory.values()).filter(
