...
export class ErrorHandler extends EventEmitter {
  ...
  private initializeDefaultPatterns(): void {
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('timeout') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('econnrefused') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('unauthorized') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('rate limit') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('out of memory') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error) => error.message.toLowerCase().includes('validation') ||
    ...
    this.addErrorPattern({
      ...
      matcher: (error, context) => error.message.toLowerCase().includes('service unavailable') ||
  ...
  async shouldRetry(error: CategorizedError, attemptNumber: number): Promise<boolean> {
    ...
    if (error.context.agentType && this.isCircuitBreakerOpen(error.context.agentType)) {
      console.log(`[ErrorHandler] Circuit breaker open for ${error.context.agentType}, skipping retry`);
      return false;
    }
  ...
  private updateCircuitBreaker(key: string, success: boolean): void {
    ...
    if (success) {
      ...
      if (state.state === 'closed' && state.failureCount >= config.failureThreshold) {
        ...
        console.log(`[ErrorHandler] Circuit breaker ${key} opened due to ${state.failureCount} failures`);
        this.emit('circuitBreakerOpened', { key, failureCount: state.failureCount });
      } else if (state.state === 'half-open') {
        state.state = 'open';
        state.nextAttemptTime = new Date(Date.now() + config.resetTimeout);
        console.log(`[ErrorHandler] Circuit breaker ${key} reopened after failure in half-open state`);
      }
    }
  }
  ...
  private calculateEscalationLevel(severity: ErrorSeverity, context: ErrorContext): number {
    ...
    switch (severity) {
      case ErrorSeverity.LOW: level = 1; break;
      case ErrorSeverity.MEDIUM: level = 2; break;
      case ErrorSeverity.HIGH: level = 3; break;
      case ErrorSeverity.CRITICAL: level = 4; break;
    }
