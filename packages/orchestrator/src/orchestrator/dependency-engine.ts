...
export class DependencyEngine extends EventEmitter {
  ...
  private initializeDefaultDependencies(): void {
    ...
    this.addDependencyRule({
      ...
      condition: (source, target) => source === 'market-research' && target === 'customer-research',
      required: false,
      dataMapping: result => ({
  ...
  buildDependencyGraph(requiredAgents: AgentType[]): DependencyGraph {
    console.log(
      `[DependencyEngine] Building dependency graph for agents: [${requiredAgents.join(', ')}]`
    )
  ...
  private calculateTotalTime(
    ...
    return executionGroups.reduce((total, group) => {
      const groupTime = Math.max(
        ...group.map(agentType => nodes.get(agentType)?.estimatedDuration || 0)
      )
  ...
  getReadyAgents(graph: DependencyGraph, context: ExecutionContext): AgentType[] {
    ...
    graph.nodes.forEach((node, agentType) => {
      ...
      const dependenciesMet = node.dependencies.every(dep => context.completedAgents.has(dep))
  ...
  private isDependencySourceFor(agentType: AgentType, rule: DependencyRule): boolean {
    ...
    return allAgentTypes.some(targetAgent => rule.condition(agentType, targetAgent))
  }

  // Optimization methods
  optimizeGraph(
    graph: DependencyGraph,
    strategyName: string = 'parallel-execution'
  ): DependencyGraph {
  ...
  analyzeDependencyComplexity(agents: AgentType[]): {
    ...
    const maxDepth = Math.max(
      ...Array.from(graph.nodes.values()).map(node => this.calculateNodeDepth(node, graph.nodes))
    )

    const parallelismOpportunities = graph.executionGroups.filter(group => group.length > 1).length
