...
export class ResultAggregator extends EventEmitter {
    ...
    this.validationRules = [
      ...
      },
      {
        ...
        validate: result => {
          ...
          const expectedConfidence = Object.entries(scoreConfidenceMap).find(
            ([_, matches]) => matches
          )?.[0]
          ...
          return {
            ...
            issues: valid
              ? []
              : [`Confidence ${result.confidence} inconsistent with score ${result.score}`],
          }
        },
        severity: 'info',
      },
      {
        ...
        validate: result => {
          ...
          const missing = requiredFields.filter(
            field => !result.metadata || result.metadata[field] === undefined
          )
  ...
  async aggregateResults(
    ...
    console.log(
      `[ResultAggregator] Aggregating ${agentResults.length} results for evaluation: ${evaluationId}`
    )
    ...
    if (validResults.length < strategy.minimumAgents) {
      throw new Error(
        `Insufficient valid results: ${validResults.length}, minimum required: ${strategy.minimumAgents}`
      )
    }
    ...
    const result: AggregatedResult = {
      ...
      metadata: {
        ...
        failedAgents: agentResults.filter(r => !validResults.includes(r)).map(r => r.agentType),
    ...
    console.log(
      `[ResultAggregator] Aggregation complete: score=${result.overallScore}, confidence=${result.confidence}`
    )
  ...
  private standardConfidenceCalculation(
  ...
  private conservativeConfidenceCalculation(
    results: AgentResponse[],
    consensus: number
  ): 'high' | 'medium' | 'low' {
  ...
  private optimisticConfidenceCalculation(
    results: AgentResponse[],
    consensus: number
  ): 'high' | 'medium' | 'low' {
  ...
  private async generateSummary(results: AgentResponse[]): Promise<AggregatedResult['summary']> {
    ...
    const categorizeInsights = (insights: string[], keywords: string[]) => {
      return insights
        .filter(insight =>
          keywords.some(keyword => insight.toLowerCase().includes(keyword.toLowerCase()))
        )
  ...
  private calculateAgentContributions(
    results: AgentResponse[],
    strategy: AggregationStrategy
  ): AggregatedResult['agentContributions'] {
  ...
  private validateResults(
    results: AgentResponse[]
  ): Map<string, { valid: boolean; issues: string[] }> {
  ...
  private logValidationIssues(
    validationSummary: Map<string, { valid: boolean; issues: string[] }>
  ): void {
  ...
  private calculateDataReliability(
    results: AgentResponse[],
    validationSummary: Map<string, any>
  ): number {
    ...
    const avgDataQuality =
      validResults.reduce((sum, result) => sum + this.assessDataQuality(result), 0) /
      validResults.length
