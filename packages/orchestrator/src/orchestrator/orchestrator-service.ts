...
export class OrchestratorService extends EventEmitter implements MessageHandler {
  ...
  canHandle(
    message: Message
  ): message is EvaluationCompleteMessage | EvaluationErrorMessage | EvaluationProgressMessage {
    return [
  ...
  async requestEvaluation(request: EvaluationRequest): Promise<string> {
    ...
    console.log(
      `Evaluation requested: ${evaluationId} for business idea: ${request.businessIdeaId}`
    )
  ...
  private async processEvaluationRequest(data: EvaluationRequestJobData): Promise<void> {
    ...
    try {
      ...
      const evaluationStartMessage = this.messageBus.createMessage(MessageType.EVALUATION_START, {
        evaluationId,
        businessIdea,
        priority:
          data.priority === JobPriority.HIGH
  ...
  private calculateConfidence(
    agentResults: Array<{ agentType: string; score: number }>
  ): 'high' | 'medium' | 'low' {
  ...
  private generateSummary(
    agentResults: Array<{ agentType: string; insights: string[]; score: number }>
  ): {
    strengths: string[]
    weaknesses: string[]
  ...
  private updateProcessingTime(processingTime: number): void {
    ...
    if (totalCompleted === 1) {
      ...
      this.metrics.averageProcessingTime =
        (this.metrics.averageProcessingTime * (totalCompleted - 1) + processingTime) /
        totalCompleted
    }
  }
  ...
  private updateAverageMetrics(result: EvaluationResult): void {
    ...
    const totalCompleted = this.metrics.completedEvaluations
    this.metrics.averageScore =
      (this.metrics.averageScore * (totalCompleted - 1) + result.overallScore) / totalCompleted
