...
export class MarketResearchAgent extends BaseAgent {
  ...
  async execute(
    ...
    try {
      console.log(`Enhanced Market Research Agent executing for business idea: ${request.businessIdea.title}`);
      ...
      console.log(`[MarketResearchAgent] Analysis completed in ${processingTime}ms with opportunity score: ${opportunityScore.overall}`);
  ...
  private inferCategory(description: string): string {
    ...
    if (keywords.includes('technology') || keywords.includes('software') || keywords.includes('app')) return 'technology';
  ...
  private async analyzeCompetitiveLandscape(request: MarketSizingRequest): Promise<CompetitiveLandscape> {
    ...
    return {
      competitorCount,
      marketConcentration: competitorCount > 20 ? 'fragmented' : competitorCount > 10 ? 'concentrated' : 'monopolistic',
      entryBarriers: {
        overall: 'medium',
        financial: 60,
      ...
      marketShare: [
        { company: 'Market Leader A', share: 25, revenue: 1000000000, growth: 8, source: 'Industry Report' },
        { company: 'Competitor B', share: 15, revenue: 600000000, growth: 12, source: 'Industry Report' },
  ...
  private extractEnhancedInsights(output: MarketResearchOutput, intelligence?: any): string[] {
    ...
    insights.push(`Total Addressable Market (TAM): ${this.formatCurrency(tam.value)} using ${tam.methodology} approach (${tam.confidence}% confidence)`);
    insights.push(`Serviceable Addressable Market (SAM): ${this.formatCurrency(sam.value)} representing addressable segment`);
    insights.push(`Serviceable Obtainable Market (SOM): ${this.formatCurrency(som.value)} as realistic capture potential`);

    // Growth insights
    const growthRate = output.marketSize.growthRate.annual;
    const growthDescription = growthRate > 15 ? 'exceptionally high' : 
                             growthRate > 10 ? 'high' :
                             growthRate > 5 ? 'moderate' : 'low';
    insights.push(`Market growing at ${growthDescription} rate of ${growthRate.toFixed(1)}% annually`);
    ...
    insights.push(`Market has ${competitive.competitorCount} competitors with ${competitive.marketConcentration} structure`);
    ...
    insights.push(`Analysis confidence: ${output.metadata.dataQuality}/100 data quality, ${output.metadata.completeness}/100 completeness`);

    // Intelligence analysis insights
    if (intelligence && intelligence.predictiveInsights && intelligence.predictiveInsights.length > 0) {
      const topInsight = intelligence.predictiveInsights[0];
      insights.push(`AI Prediction: ${topInsight.prediction} (${topInsight.confidence}% confidence)`);
    }
    ...
    if (intelligence && intelligence.riskAnalysis && intelligence.riskAnalysis.length > 0) {
      ...
      insights.push(`Key Risk: ${topRisk.description} (${topRisk.riskScore.toFixed(0)}/100 risk score)`);
    }
  ...
  private calculateCompleteness(marketSize: any, trends: MarketTrend[], competitive: CompetitiveLandscape): number {
    let completeness = 0;
    let maxScore = 0;
  ...
  private identifyLimitations(confidenceAssessment: any): string[] {
    ...
    if (confidenceAssessment.components.dataRecency < 70) {
      limitations.push('Analysis includes some older market data that may not reflect current conditions');
    }
  ...
  private generateRecommendations(opportunityScore: any, confidenceAssessment: any): any[] {
    ...
    if (opportunityScore.overall >= 80) {
      recommendations.push({
        ...
        rationale: 'High opportunity score indicates favorable market conditions across multiple dimensions'
      });
