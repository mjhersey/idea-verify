...
export class AgentCoordinator implements MessageHandler {
  ...
  private getDependents(agentType: AgentType, dependencies: AgentDependency[]): AgentType[] {
    return dependencies.filter(dep => dep.dependsOn.includes(agentType)).map(dep => dep.agentType)
  }
  ...
  private async startAgent(
    ...
    const message = this.messageBus.createMessage(MessageType.AGENT_START, {
      evaluationId,
      agentType,
      businessIdea,
  ...
  private async handleAgentError(message: AgentErrorMessage): Promise<void> {
    ...
    console.error(
      `[AgentCoordinator] Agent error: ${agentType} for evaluation: ${evaluationId}`,
      error
    )
  ...
  private async handleAgentDataRequest(message: AgentDataRequestMessage): Promise<void> {
    ...
    console.log(
      `[AgentCoordinator] Data request: ${requestingAgent} requesting ${dataType} from ${targetAgent}`
    )
  ...
  private async notifyDependentAgents(
    ...
    if (dependentAgents.length > 0) {
      const message = this.messageBus.createMessage(MessageType.AGENT_DEPENDENCY_COMPLETE, {
        evaluationId,
        completedAgent,
        dependentAgents,
        result,
  ...
  private async publishWorkflowProgress(evaluationId: string): Promise<void> {
    ...
    const overallProgress =
      totalAgents > 0 ? Math.round((completedAgents.length / totalAgents) * 100) : 0

    const message = this.messageBus.createMessage(MessageType.EVALUATION_PROGRESS, {
      evaluationId,
      overallProgress,
      completedAgents,
      runningAgents,
  ...
  private async checkWorkflowCompletion(evaluationId: string): Promise<void> {
    ...
    if (allComplete) {
      ...
      const results = states.filter(s => s.result).map(s => s.result!)
      ...
      if (workflow.status === 'completed') {
        const message = this.messageBus.createMessage(MessageType.EVALUATION_COMPLETE, {
          evaluationId,
          results,
        ...
        const message = this.messageBus.createMessage(MessageType.EVALUATION_ERROR, {
          evaluationId,
          error: {
