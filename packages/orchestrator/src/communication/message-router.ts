...
export class MessageRouter extends EventEmitter {
  ...
  unregisterHandler(name: string): boolean {
    ...
    if (removed) {
      ...
      const dependentRoutes = Array.from(this.routes.entries())
        .filter(([_, route]) => route.rule.destination.handler === name);
  ...
  private async routeToHandler(message: Message, route: MessageRoute): Promise<void> {
    try {
      ...
      route.errorCount++;
      console.error(`[MessageRouter] Error routing to handler ${route.rule.destination.handler}:`, error);
  ...
  private async processFanoutPatterns(message: Message): Promise<void> {
    const matchingPatterns = Array.from(this.fanoutPatterns.values())
      .filter(pattern => 
  ...
  getMessageHistory(limit?: number): Message[] {
    return limit 
      ? this.messageHistory.slice(-limit)
      : [...this.messageHistory];
  }
