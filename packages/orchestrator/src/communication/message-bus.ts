...
export class MessageBus extends EventEmitter implements IMessageBus {
  ...
  async subscribe<T extends Message>(
    ...
    console.log(
      `[MessageBus] Subscribed handler to ${messageType} (${handlersSet.size} total handlers)`
    )
  }
  ...
  createMessage(type: MessageType, payload: any, options: Partial<BaseMessage> = {}): Message {
    return {
      id: uuidv4(),
      timestamp: new Date(),
      type,
