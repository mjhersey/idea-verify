...
export class MultiAgentQueueManager extends EventEmitter {
  ...
  async addAgentExecution(agentType: AgentType, data: AgentExecutionJob, options?: JobOptions): Promise<string> {
    const queue = this.agentQueues.get(agentType);
    if (!queue) {
  ...
  private async processAgentExecution(agentType: AgentType, data: AgentExecutionJob): Promise<any> {
    console.log(`[MultiAgentQueueManager] Executing ${agentType} for evaluation: ${data.evaluationId}`);
    ...
    try {
      ...
      console.log(`[MultiAgentQueueManager] ${agentType} execution completed with score: ${result.score}`);
      return result;
  ...
  private extractDependenciesForAgent(agentType: AgentType, allDependencies: Record<AgentType, AgentType[]>): Record<string, any> {
  ...
  private async handleFailedJob(agentType: AgentType, jobId: string, error: any): Promise<void> {
    ...
    try {
      await this.deadLetterQueue.add('failed-agent-job' as any, {
        originalJobId: jobId,
        agentType,
        error: error.message,
        timestamp: new Date(),
