...
export class PostgresAgentResultRepository {
  ...
  async create(data: CreateAgentResultInput): Promise<AgentResult> {
    ...
    const values = [data.evaluation_id, data.agent_type, JSON.stringify(data.input_data || null)]
    ...
    console.log(
      `Created agent result: ${rows[0].id} for evaluation: ${data.evaluation_id}, agent: ${data.agent_type}`
    )
    return rows[0]
  }
