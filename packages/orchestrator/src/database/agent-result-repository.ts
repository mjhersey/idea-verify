...
export class AgentResultRepository {
  ...
  async create(data: CreateAgentResultInput): Promise<AgentResult> {
    ...
    console.log(
      `Created agent result: ${agentResult.id} for evaluation: ${data.evaluation_id}, agent: ${data.agent_type}`
    )
  ...
  async findByEvaluationId(evaluationId: string): Promise<AgentResult[]> {
    return Array.from(this.agentResults.values()).filter(
      result => result.evaluation_id === evaluationId
    )
  }

  async findByAgentType(agentType: AgentType): Promise<AgentResult[]> {
    return Array.from(this.agentResults.values()).filter(result => result.agent_type === agentType)
  }

  async findByEvaluationAndAgent(
    evaluationId: string,
    agentType: AgentType
  ): Promise<AgentResult | null> {
    const results = Array.from(this.agentResults.values())
    const result = results.find(r => r.evaluation_id === evaluationId && r.agent_type === agentType)
    return result || null
  }
  ...
  async getSuccessRates(): Promise<
    Record<AgentType, { total: number; success: number; rate: number }>
  > {
    const results = Array.from(this.agentResults.values())
