...
export class ValidationScoringEngine {
  ...
  async calculateValidationScore(
    request: CustomerResearchRequest
  ): Promise<ValidationScoringResult> {
    const startTime = Date.now()
    console.log(
      `[ValidationScoringEngine] Calculating validation score for: ${request.businessIdea.title}`
    )
    ...
    try {
      ...
      console.log(
        `[ValidationScoringEngine] Validation scoring completed in ${Date.now() - startTime}ms`
      )
      ...
      throw new Error(
        `Validation scoring failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private async calculateProblemValidationScore(request: CustomerResearchRequest): Promise<number> {
    ...
    if (
      ...
      description.includes('inefficient') ||
  ...
  private async calculateSolutionFitScore(request: CustomerResearchRequest): Promise<number> {
    ...
    if (
      description.includes('ai') ||
      description.includes('personalized') ||
  ...
  private async calculateWillingnessToPayScore(request: CustomerResearchRequest): Promise<number> {
    ...
    const description = request.businessIdea.description.toLowerCase()
    if (
      description.includes('save time') ||
  ...
  private async calculateCompetitiveAdvantageScore(
    request: CustomerResearchRequest
  ): Promise<number> {
    const domain = this.extractDomain(request.businessIdea.description)
    const description = request.businessIdea.description.toLowerCase()
    ...
    if (
      ...
      description.includes('breakthrough')
    ) {
      score += 15
    } else if (
    ...
    if (
      description.includes('patent') ||
      description.includes('proprietary') ||
  ...
  private calculateComponentWeights(request: CustomerResearchRequest) {
    ...
    if (domain === 'business') {
      weights.willingnessToPayScore = 0.2 // Business focus on ROI
      weights.competitiveAdvantage = 0.15
      weights.segmentViability = 0.1
  ...
  private getDomainContext(domain: string): string {
    const contextMap = {
      'health-fitness':
        'In the health and fitness market, customer motivation and engagement are critical success factors.',
      business:
        'For business solutions, demonstrating clear ROI and operational efficiency is essential.',
      technology:
        'In the technology space, technical differentiation and developer adoption drive success.',
      finance:
        'Financial services require strong trust, regulatory compliance, and transparent value proposition.',
  ...
  private generateRecommendations(
    ...
    if (components.competitiveAdvantage < 70) {
      recommendations.push(
        'Strengthen competitive differentiation through unique value propositions'
      )
    }
  ...
  private getCompetitiveAdvantageRationale(
    score: number,
    request: CustomerResearchRequest
  ): string {
  ...
  private identifyAssumptionRisks(
    validationScore: CustomerValidationScore,
    request: CustomerResearchRequest
  ): string[] {
    const risks = []
    ...
    if (
      validationScore.components.problemValidation > 80 &&
      validationScore.components.marketDemand < 60
    ) {
      risks.push(
        'Strong problem validation but weak market demand signals may indicate timing issues'
      )
    }

    if (
      validationScore.components.solutionFit > 80 &&
      validationScore.components.competitiveAdvantage < 50
    ) {
