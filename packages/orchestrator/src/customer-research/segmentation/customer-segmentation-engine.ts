...
export class CustomerSegmentationEngine {
  ...
  async analyzeCustomerSegments(
    ...
    console.log(
      `[CustomerSegmentationEngine] Analyzing customer segments for: ${request.businessIdea.title}`
    )

    try {
      // Parallel analysis of different segmentation approaches
      const [demographicSegments, psychographicSegments, behavioralSegments] = await Promise.all([
      ...
      const allSegments = this.mergeAndRefineSegments(
        [...demographicSegments, ...psychographicSegments, ...behavioralSegments],
        request
      )
      ...
      const segmentationMetrics = {
        ...
        totalMarketSize: segmentsWithMarketSizing.reduce(
          (sum, s) => sum + s.sizeEstimation.totalAddressableMarket,
          0
        ),
      ...
      console.log(
        `[CustomerSegmentationEngine] Segmentation completed in ${Date.now() - startTime}ms`
      )
      ...
      throw new Error(
        `Customer segmentation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }

  private async analyzeDemographicSegments(
    request: CustomerResearchRequest
    ...
    if (
      ...
      businessIdea.description.toLowerCase().includes('business')
    ) {
  ...
  private async analyzePsychographicSegments(
    request: CustomerResearchRequest
  ): Promise<CustomerSegment[]> {
    const businessIdea = request.businessIdea
    const segments: CustomerSegment[] = []
    ...
    if (
      businessIdea.description.toLowerCase().includes('health') ||
    ...
    if (
      businessIdea.description.toLowerCase().includes('tech') ||
    ...
    if (
      businessIdea.description.toLowerCase().includes('environment') ||
  ...
  private async analyzeBehavioralSegments(
    request: CustomerResearchRequest
  ): Promise<CustomerSegment[]> {
    const businessIdea = request.businessIdea
    const segments: CustomerSegment[] = []
    ...
    if (
      businessIdea.description.toLowerCase().includes('professional') ||
  ...
  private createEnvironmentallyConsciousSegment(businessIdea: any): CustomerSegment {
    return {
      ...
      description:
        'Consumers who prioritize sustainability and environmental impact in purchasing decisions',
      demographics: {
        ageRange: '25-45',
        income: '$50,000-$120,000',
        education: 'College-educated',
  ...
  private mergeAndRefineSegments(
    segments: CustomerSegment[],
    request: CustomerResearchRequest
  ): CustomerSegment[] {
    ...
    return segments.map(segment => {
      ...
      return {
        ...
        sizeEstimation: {
          ...
          totalAddressableMarket: Math.round(
            segment.sizeEstimation.totalAddressableMarket * sizingMultiplier
          ),
          confidence: Math.min(
            100,
            segment.sizeEstimation.confidence + (sizingMultiplier > 1 ? -5 : 0)
          ),
        },
      }
    })
  ...
  private calculateSegmentationConfidence(segments: CustomerSegment[]): number {
    ...
    const sizeConfidence =
      segments.reduce((sum, s) => sum + s.sizeEstimation.confidence, 0) / segments.length
