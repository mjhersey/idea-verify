...
import { PainPointAnalysis, DataSource, CustomerResearchRequest } from '../schemas/customer-research-types.js';
...
export class PainPointAnalysisEngine {
  ...
  async analyzePainPoints(request: CustomerResearchRequest): Promise<PainPointAnalysisResult> {
    ...
    console.log(`[PainPointAnalysisEngine] Analyzing pain points for: ${request.businessIdea.title}`);
    ...
    try {
      ...
      const painPoints = this.synthesizePainPoints(
        customerComplaints, journeyAnalysis, needsMapping, request
      );
      ...
      console.log(`[PainPointAnalysisEngine] Pain point analysis completed in ${Date.now() - startTime}ms`);
      ...
      throw new Error(`Pain point analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private getCommonComplaints(domain: string, category: string): Array<{complaint: string, severity: string, frequency: string}> {
    const complaintMap = {
      'health-fitness': [
        { complaint: 'Lack of personalized workout plans', severity: 'high', frequency: 'frequent' },
        { complaint: 'Difficulty maintaining motivation', severity: 'high', frequency: 'constant' },
        { complaint: 'Expensive personal training costs', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Generic fitness apps that don\'t adapt', severity: 'medium', frequency: 'frequent' },
      ...
      'business': [
        { complaint: 'Manual processes waste too much time', severity: 'high', frequency: 'constant' },
        { complaint: 'Poor integration between business tools', severity: 'high', frequency: 'frequent' },
        { complaint: 'High software licensing costs', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Difficult to scale operations efficiently', severity: 'high', frequency: 'frequent' },
        { complaint: 'Lack of real-time business insights', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Complex onboarding for new tools', severity: 'medium', frequency: 'occasional' }
      ],
      'technology': [
        ...
        { complaint: 'Performance bottlenecks in production', severity: 'high', frequency: 'occasional' },
        { complaint: 'Limited customization options', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Steep learning curve for new tools', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Lack of integration with existing stack', severity: 'high', frequency: 'frequent' }
      ],
      'finance': [
        ...
        { complaint: 'Lack of personalized financial advice', severity: 'medium', frequency: 'frequent' },
        { complaint: 'Limited educational resources', severity: 'low', frequency: 'frequent' },
        { complaint: 'Difficulty tracking financial goals', severity: 'medium', frequency: 'frequent' }
      ],
      'education': [
        { complaint: 'Outdated curriculum content', severity: 'high', frequency: 'frequent' },
        { complaint: 'Lack of practical, hands-on learning', severity: 'high', frequency: 'frequent' },
        { complaint: 'High cost of quality education', severity: 'high', frequency: 'constant' },
        { complaint: 'Limited personalized learning paths', severity: 'medium', frequency: 'frequent' },
    ...
    return complaintMap[domain] || [
      { complaint: 'Current solutions don\'t meet specific needs', severity: 'medium', frequency: 'frequent' },
  ...
  private generateCustomerQuotes(domain: string, businessIdea: any) {
    const quoteMap = {
      'health-fitness': [
        {
          quote: "I've tried so many fitness apps but none of them actually understand my schedule or fitness level. They're all one-size-fits-all.",
          source: "Reddit r/fitness",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "Personal trainers are too expensive, but generic workout plans don't work for me. I need something in between.",
          source: "MyFitnessPal Community",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "I lose motivation after a few weeks because I don't see real progress. The apps don't adapt to what's actually working.",
      ...
      'business': [
        {
          quote: "We're spending 3-4 hours a day on manual data entry that should be automated. It's killing our productivity.",
          source: "Small Business Forum",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "Our CRM doesn't talk to our accounting software, which doesn't talk to our project management tool. It's a nightmare.",
          source: "Entrepreneur Community",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "Every business tool we try has a steep learning curve. By the time we're productive, we've wasted months.",
      ...
      'technology': [
        {
          quote: "Setting up our development environment takes new hires 2-3 days. There has to be a better way.",
          source: "Stack Overflow",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "The documentation is terrible and support is non-existent. We're basically figuring everything out ourselves.",
          source: "GitHub Discussion",
          anonymized: true,
          sentiment: 'negative' as const
        },
        {
          quote: "Performance monitoring tools give us data but no actionable insights. We're drowning in metrics.",
    ...
    return quoteMap[domain] || [
      {
        quote: "Current solutions just don't address our specific pain points. We need something better.",
  ...
  private identifyJourneyStages(domain: string): string[] {
    const stageMap = {
      'health-fitness': ['Awareness of fitness need', 'Research fitness options', 'Try initial solution', 'Experience challenges', 'Seek better alternatives', 'Consider giving up'],
      'business': ['Identify business problem', 'Research solutions', 'Evaluate options', 'Implementation', 'Integration challenges', 'Optimization'],
      'technology': ['Problem identification', 'Tool research', 'Evaluation', 'Setup and configuration', 'Learning and adoption', 'Optimization'],
      'finance': ['Financial awareness', 'Research options', 'Compare services', 'Initial signup', 'Ongoing usage', 'Review and optimize'],
      'education': ['Learning need identification', 'Course research', 'Enrollment', 'Learning process', 'Application', 'Career impact'],
      'general': ['Problem awareness', 'Solution research', 'Evaluation', 'Implementation', 'Usage', 'Optimization']
    };
  ...
  private mapTouchpointPains(domain: string) {
    const touchpointMap = {
      ...
      'business': [
        ...
        { touchpoint: 'Implementation', pain: 'Poor integration with existing tools', severity: 85 },
  ...
  private identifyCriticalMoments(domain: string): string[] {
    const momentsMap = {
      'health-fitness': ['First week of using fitness app', 'Plateau in results after 1 month', 'Considering gym membership vs app'],
      'business': ['System integration failure', 'Team adoption resistance', 'ROI evaluation after 3 months'],
      'technology': ['Production deployment issues', 'Performance problems under load', 'Team onboarding difficulties'],
      'finance': ['First major transaction', 'Fee structure realization', 'Customer service interaction'],
      'education': ['First practical application', 'Mid-course engagement drop', 'Career outcome evaluation'],
      'general': ['Initial implementation', 'First major challenge', 'Value assessment']
    };
    return momentsMap[domain] || momentsMap['general'];
  }

  private analyzeDropoffPoints(domain: string): Array<{stage: string, dropoffRate: number, reason: string}> {
    const dropoffMap = {
      'health-fitness': [
  ...
  private identifyFrustrationTriggers(domain: string): string[] {
    const triggerMap = {
      'health-fitness': ['No visible progress', 'Boring repetitive workouts', 'Lack of guidance', 'Time constraints'],
      'business': ['System downtime', 'Data sync failures', 'Poor customer support', 'Hidden costs'],
      'technology': ['Deployment failures', 'Performance degradation', 'Documentation gaps', 'Breaking changes'],
      'finance': ['Unexpected fees', 'Account restrictions', 'Poor communication', 'Complex processes'],
  ...
  private identifyEmotionalNeeds(domain: string, category: string) {
    const emotionalMap = {
      'health-fitness': ['Confidence building', 'Motivation and encouragement', 'Sense of achievement', 'Community support'],
      'business': ['Control and visibility', 'Confidence in decisions', 'Reduced stress', 'Professional growth'],
      'technology': ['Mastery and competence', 'Innovation satisfaction', 'Problem-solving fulfillment', 'Professional recognition'],
      'finance': ['Security and safety', 'Control over finances', 'Confidence in decisions', 'Peace of mind'],
      'education': ['Sense of progress', 'Competence building', 'Career confidence', 'Intellectual satisfaction'],
  ...
  private identifySocialNeeds(domain: string, category: string) {
    const socialMap = {
      'health-fitness': ['Community support', 'Sharing achievements', 'Workout accountability', 'Expert guidance'],
      'business': ['Team collaboration', 'Industry recognition', 'Professional networking', 'Knowledge sharing'],
      'technology': ['Developer community', 'Knowledge sharing', 'Professional recognition', 'Mentorship'],
      'finance': ['Financial advisor access', 'Peer learning', 'Family financial security', 'Community trust'],
      'education': ['Peer learning', 'Instructor support', 'Professional networking', 'Career community'],
  ...
  private analyzeCurrentSolutions(domain: string, category: string) {
    const solutionMap = {
      'health-fitness': [
        { solution: 'Generic fitness apps', satisfaction: 40, limitations: ['One-size-fits-all', 'No personalization', 'Poor motivation'] },
        { solution: 'Personal trainers', satisfaction: 80, limitations: ['Too expensive', 'Limited availability', 'Travel required'] },
        { solution: 'Gym memberships', satisfaction: 60, limitations: ['Monthly fees', 'Crowded facilities', 'Limited guidance'] }
      ],
      'business': [
        { solution: 'Manual processes', satisfaction: 20, limitations: ['Time consuming', 'Error prone', 'Not scalable'] },
        { solution: 'Enterprise software', satisfaction: 65, limitations: ['Too expensive', 'Complex setup', 'Over-engineered'] },
        { solution: 'Multiple point solutions', satisfaction: 45, limitations: ['Poor integration', 'Data silos', 'High complexity'] }
      ],
      'technology': [
        { solution: 'Open source tools', satisfaction: 60, limitations: ['Complex setup', 'Limited support', 'Maintenance overhead'] },
        { solution: 'Enterprise platforms', satisfaction: 70, limitations: ['High cost', 'Vendor lock-in', 'Over-complexity'] },
        { solution: 'Custom development', satisfaction: 50, limitations: ['High cost', 'Long timeline', 'Maintenance burden'] }
      ]
    };
    return solutionMap[domain] || [
      { solution: 'Existing solutions', satisfaction: 50, limitations: ['Limited functionality', 'High cost', 'Poor UX'] }
    ];
  }
  ...
  private identifyExistingSolutions(domain: string): string[] {
    const solutionMap = {
      'health-fitness': ['Fitness apps', 'Personal trainers', 'Gym memberships', 'Online fitness programs', 'Wearable devices'],
      'business': ['CRM systems', 'ERP software', 'Project management tools', 'Automation platforms', 'Analytics tools'],
      'technology': ['Development frameworks', 'Monitoring tools', 'CI/CD platforms', 'Cloud services', 'Open source libraries'],
      'finance': ['Banking apps', 'Investment platforms', 'Budgeting tools', 'Financial advisors', 'Robo-advisors'],
      'education': ['Online courses', 'Learning management systems', 'Educational apps', 'Bootcamps', 'Traditional education'],
      'general': ['Existing software', 'Manual processes', 'Consultant services', 'Online tools', 'Custom solutions']
    };
  ...
  private analyzeSolutionLimitations(domain: string): string[] {
    const limitationMap = {
      'health-fitness': ['Lack of personalization', 'Poor motivation features', 'Limited progress tracking', 'No real-time guidance'],
      'business': ['Poor integration', 'High complexity', 'Expensive licensing', 'Limited customization'],
      ...
      'education': ['Outdated content', 'Poor engagement', 'Limited practical application', 'High costs'],
  ...
  private identifyUnaddressedProblems(domain: string, solution: string): string[] {
    const problemMap = {
      'health-fitness': ['Real-time form correction', 'AI-powered personalization', 'Contextual motivation', 'Integrated lifestyle tracking'],
      'business': ['True end-to-end automation', 'Intelligent business insights', 'Seamless tool integration', 'Predictive analytics'],
      'technology': ['Zero-configuration setup', 'Intelligent performance optimization', 'Proactive issue detection', 'Automated scaling'],
      'finance': ['Transparent fee structures', 'Personalized investment strategies', 'Real-time financial coaching', 'Integrated life planning'],
      'education': ['Adaptive learning paths', 'Real-world project integration', 'Career outcome tracking', 'Personalized mentorship'],
      'general': ['True personalization', 'Seamless integration', 'Predictive capabilities', 'Intelligent automation']
    };
  ...
  private analyzeCompetitorWeaknesses(domain: string): string[] {
    const weaknessMap = {
      'health-fitness': ['Generic approaches', 'Poor user retention', 'Limited data utilization', 'Weak community features'],
      'business': ['Complex user interfaces', 'Poor integration capabilities', 'High implementation costs', 'Limited scalability'],
      'technology': ['Steep learning curves', 'Performance bottlenecks', 'Poor developer experience', 'Limited customization'],
      'finance': ['Trust and transparency issues', 'Poor customer service', 'Limited educational resources', 'Complex fee structures'],
      'education': ['Low engagement rates', 'Poor completion rates', 'Limited career support', 'Outdated methodologies'],
      'general': ['Poor user experience', 'Limited customization', 'High switching costs', 'Weak customer support']
    };
  ...
  private identifyOpportunityAreas(domain: string, solution: string): string[] {
    const opportunityMap = {
      'health-fitness': ['AI-powered personalization', 'Community-driven motivation', 'Integrated health ecosystem', 'Real-time coaching'],
      'business': ['No-code automation', 'AI-driven insights', 'Unified business platform', 'Predictive optimization'],
      'technology': ['Developer experience focus', 'Zero-configuration solutions', 'Intelligent automation', 'Performance by default'],
      'finance': ['Transparent and fair pricing', 'Educational first approach', 'Integrated life planning', 'Community-driven advice'],
      'education': ['Outcome-focused learning', 'Real-world application', 'Personalized career paths', 'Continuous skill tracking'],
      'general': ['User-centric design', 'Transparent pricing', 'Community integration', 'Intelligent automation']
    };
    return opportunityMap[domain] || opportunityMap['general'];
  }

  private synthesizePainPoints(customerComplaints: any, journeyAnalysis: any, needsMapping: any, request: CustomerResearchRequest): PainPointAnalysis[] {
    ...
    complaints.forEach((complaint: any, index: number) => {
      const painPoint: PainPointAnalysis = {
        ...
        impact: {
          ...
          emotional: this.assessEmotionalImpact(complaint.complaint, customerComplaints.emotionalIndicators)
        },
        opportunity: this.calculateOpportunityScore(complaint.severity, complaint.frequency, domain),
  ...
  private generateProblemDescription(problem: string, domain: string): string {
    const descriptionMap = {
      'Lack of personalized workout plans': 'Users struggle with generic fitness programs that don\'t account for their individual fitness level, preferences, or constraints.',
      'Difficulty maintaining motivation': 'Customers lose interest and stop using fitness solutions due to lack of engagement and accountability.',
      'Manual processes waste too much time': 'Businesses spend excessive time on repetitive tasks that could be automated, reducing overall productivity.',
      'Poor integration between business tools': 'Companies use multiple software solutions that don\'t communicate effectively, creating data silos and inefficiencies.'
    };
    return descriptionMap[problem] || `Customers experience significant challenges with ${problem.toLowerCase()}, impacting their overall satisfaction and outcomes.`;
  }
  ...
  private calculateEvidenceStrength(severity: string, frequency: string): number {
    ...
    const frequencyScore = frequency === 'constant' ? 35 : frequency === 'frequent' ? 25 : frequency === 'occasional' ? 15 : 5;
  ...
  private assessOperationalImpact(problem: string): string {
    ...
    } else if (problem.toLowerCase().includes('complex') || problem.toLowerCase().includes('difficult')) {
  ...
  private assessEmotionalImpact(problem: string, emotionalIndicators: string[]): string {
    const negativeEmotions = emotionalIndicators.filter(emotion => 
  ...
  private calculateOpportunityScore(severity: string, frequency: string, domain: string): number {
    const severityScore = severity === 'high' ? 40 : severity === 'medium' ? 25 : 10;
    const frequencyScore = frequency === 'constant' ? 30 : frequency === 'frequent' ? 20 : frequency === 'occasional' ? 10 : 5;
  ...
  private calculatePainPointMetrics(painPoints: PainPointAnalysis[]) {
    ...
    const criticalPainPoints = painPoints.filter(p => p.severity === 'high' && (p.frequency === 'frequent' || p.frequency === 'constant')).length;
    
    const severityScores = painPoints.map(p => p.severity === 'high' ? 3 : p.severity === 'medium' ? 2 : 1);
    const averageSeverityScore = Math.round((severityScores.reduce((sum, score) => sum + score, 0) / totalPainPoints) * 33.33);
    
    const mostFrequentPain = painPoints
    ...
    const overallOpportunityScore = Math.round(painPoints.reduce((sum, p) => sum + p.opportunity, 0) / totalPainPoints);
  ...
  private generateCustomerJourney(journeyAnalysis: any, painPoints: PainPointAnalysis[], request: CustomerResearchRequest) {
    const touchpoints = journeyAnalysis.touchpointPains.map((tp: any) => ({
      stage: tp.touchpoint,
      painPoints: [tp.pain],
      severity: tp.severity,
      frequency: 'frequent',
      customerImpact: tp.severity > 70 ? 'High impact on customer experience' : 'Moderate impact on customer experience'
    }));

    return {
      touchpoints,
      criticalMoments: journeyAnalysis.criticalMoments,
      opportunityAreas: this.identifyOpportunityAreas(this.extractDomain(request.businessIdea.description), request.businessIdea.description)
    };
  }

  private createNeedsAnalysis(needsMapping: any, painPoints: PainPointAnalysis[], request: CustomerResearchRequest) {
    const primaryNeeds = needsMapping.functionalNeeds.map((need: any) => ({
      ...
      customerSegments: this.identifyAffectedSegments(need.need, this.extractDomain(request.businessIdea.description))
    }));
  ...
  private assessSolutionFit(solutionGapAnalysis: any, painPoints: PainPointAnalysis[], request: CustomerResearchRequest) {
    const addressableProblems = painPoints
      .filter(p => p.opportunity > 60)
      .map(p => p.problem);

    const problemSolutionAlignment = Math.min(100, Math.round(
      (addressableProblems.length / painPoints.length) * 100
    ));
    ...
    return [
      {
        ...
        recency: new Date(),
        accessDate: new Date(),
        dataPoints: ['customer complaints', 'problem frequency', 'severity levels', 'emotional indicators'],
        sampleSize: 7500,
      ...
      },
      {
        ...
        recency: new Date(),
        accessDate: new Date(),
        dataPoints: ['existing solutions', 'solution limitations', 'competitor weaknesses', 'opportunity areas'],
