...
export class PersonaRecommendationEngine {
  ...
  async generatePersonaRecommendations(
    request: CustomerResearchRequest
  ): Promise<PersonaRecommendationResult> {
    const startTime = Date.now()
    console.log(
      `[PersonaRecommendationEngine] Generating persona recommendations for: ${request.businessIdea.title}`
    )

    try {
      // Parallel analysis of persona generation approaches
      const [demographicPersonas, behavioralPersonas, needsBasedPersonas, marketSegmentPersonas] =
        await Promise.all([
      ...
      console.log(
        `[PersonaRecommendationEngine] Persona generation completed in ${Date.now() - startTime}ms`
      )
  ...
  private async generateDemographicPersonas(
    request: CustomerResearchRequest
  ...
  private async generateBehavioralPersonas(
    request: CustomerResearchRequest
  ): Promise<CustomerPersona[]> {
    const domain = this.extractDomain(request.businessIdea.description)
  ...
  private async generateNeedsBasedPersonas(
    request: CustomerResearchRequest
  ): Promise<CustomerPersona[]> {
    const domain = this.extractDomain(request.businessIdea.description)
  ...
  private async generateMarketSegmentPersonas(
    request: CustomerResearchRequest
  ): Promise<CustomerPersona[]> {
    const domain = this.extractDomain(request.businessIdea.description)
  ...
  private createDemographicPersonas(
    domain: string,
    targetMarket: string,
    request: CustomerResearchRequest
  ): CustomerPersona[] {
    ...
    if (domain === 'health-fitness') {
      personas.push({
        ...
        background:
          'Working professional with demanding schedule who values health but struggles to maintain consistent fitness routine. Lives in urban area with access to gyms but prefers convenient, flexible solutions.',
        goals: [
        ...
        behaviors: {
          technology: [
            'Uses smartphone apps daily',
            'Prefers integrated solutions',
      ...
      personas.push({
        ...
        background:
          'Tech-savvy fitness enthusiast who has been working out for several years. Knowledgeable about fitness but looking for advanced, personalized training that adapts to his progress.',
        goals: [
      ...
      personas.push({
        ...
        background:
          'Successful small business owner running a 12-person marketing agency. Constantly looking for ways to streamline operations, reduce manual work, and scale efficiently without proportional cost increases.',
        goals: [
        ...
        behaviors: {
          technology: [
            'Adopts proven solutions',
            'Values integration',
            'Needs reliability',
            'Prefers comprehensive platforms',
          ],
          communication: [
            'Email heavy user',
            'Video calls',
        ...
        marketingInsights: {
          ...
          messaging: [
            ...
            'Proven by businesses like yours',
          ],
          contentTypes: [
            'Case studies',
            'ROI calculators',
      ...
      personas.push({
        ...
        background:
          'Experienced software developer at a mid-size tech company. Values tools that improve development speed, code quality, and team collaboration. Influences tool adoption decisions within his team.',
        goals: [
        ...
        behaviors: {
          ...
          purchasing: [
            'Evaluates free tiers first',
            'Values technical documentation',
            'Peer recommendations important',
          ],
          research: [
            'GitHub repositories',
            'Technical blogs',
  ...
  private createBehavioralPersonas(
    domain: string,
    request: CustomerResearchRequest
  ): CustomerPersona[] {
      ...
      return [
        {
          ...
          background:
            'Has tried multiple fitness solutions but struggles with consistency. Highly motivated in short bursts but needs external accountability and community support to maintain long-term habits.',
          goals: [
  ...
  private createNeedsBasedPersonas(
    domain: string,
    request: CustomerResearchRequest
  ): CustomerPersona[] {
      ...
      return [
        {
          ...
          background:
            'Founded a B2B SaaS company 2 years ago, now at 15 employees and growing rapidly. Needs scalable systems and processes to manage growth without losing agility or breaking the bank.',
          goals: [
  ...
  private createMarketSegmentPersonas(
    domain: string,
    category: string,
    request: CustomerResearchRequest
  ): CustomerPersona[] {
    ...
    if (category.includes('b2b') || category.includes('business')) {
      personas.push({
        ...
        background:
          'Senior IT professional responsible for technology decisions at a 500-person manufacturing company. Focuses on solutions that improve operational efficiency while maintaining security and compliance.',
        goals: [
  ...
  private calculatePersonaMarketSize(
  ...
  private prioritizeAndLimitPersonas(
    personas: CustomerPersona[],
    maxPersonas: number,
    request: CustomerResearchRequest
  ): CustomerPersona[] {
  ...
  private calculatePersonaMetrics(personas: CustomerPersona[], request: CustomerResearchRequest) {
    ...
    const domainMarketSize = this.getBaseDomainMarketSize(
  ...
  private estimateLifetimeValue(
    ...
    if (
      persona.demographics.occupation.includes('Owner') ||
      persona.demographics.occupation.includes('Director')
    ) {
      baseValue *= 3
    }
  ...
  private generateSegmentationRecommendations(
    personas: CustomerPersona[],
    request: CustomerResearchRequest
  ): string[] {
    ...
    if (businessPersonas.length > 0) {
      recommendations.push(
        'Develop B2B-specific features and messaging for business decision makers'
      )
    }

    const techSavvyPersonas = personas.filter(
      p =>
    ...
    return recommendations.length > 0
      ? recommendations
      : ['Focus on primary persona for initial market entry']
  }

  private identifyCustomizationOpportunities(
    personas: CustomerPersona[],
    request: CustomerResearchRequest
  ): string[] {
    const opportunities = []
    ...
    const needCounts = allNeeds.reduce(
      (acc, need) => {
    ...
    return opportunities.length > 0
      ? opportunities
      : ['Focus on core functionality that serves all personas']
  }
  ...
  private recommendValidationMethods(
    personas: CustomerPersona[],
    request: CustomerResearchRequest
  ): string[] {
    const methods = [
