...
export class WillingnessToPayEngine {
  ...
  async analyzeWillingnessToPay(request: CustomerResearchRequest): Promise<WillingnessToPayResult> {
    ...
    console.log(
      `[WillingnessToPayEngine] Analyzing willingness to pay for: ${request.businessIdea.title}`
    )
    ...
    try {
      ...
      const valuePerception = this.createValuePerceptionAnalysis(valuePerceptionAnalysis, request)
      ...
      console.log(
        `[WillingnessToPayEngine] Willingness to pay analysis completed in ${Date.now() - startTime}ms`
      )
      ...
      throw new Error(
        `Willingness to pay analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private identifyCoreValue(domain: string, solution: string): string {
    const valueMap = {
      'health-fitness':
        'Personalized AI-powered fitness guidance at a fraction of personal trainer cost',
  ...
  private identifyQuantifiableBenefits(domain: string) {
    const benefitsMap = {
      'health-fitness': [
        'Save $2000+ vs personal trainer',
        '3x faster results',
        '50% more consistent workouts',
      ],
      ...
      education: [
        ...
        'Save $5000+ vs traditional education',
      ],
  ...
  private identifyCostJustifications(domain: string) {
    const justificationMap = {
      'health-fitness': [
        ...
        {
          factor: 'Results guarantee',
          justification: 'Personalized approach works',
          acceptance: 70,
        },
      ],
      business: [
        ...
        { factor: 'Scalability', justification: 'Grow without proportional costs', acceptance: 85 },
        {
          ...
          justification: 'Stay ahead of competition',
          acceptance: 80,
        },
      ],
      technology: [
        ...
        { factor: 'Code quality', justification: 'Reduce maintenance costs', acceptance: 85 },
        {
          ...
          justification: 'Developer productivity boost',
          acceptance: 80,
        },
      ],
    }
    return (
      justificationMap[domain] || [
  ...
  private analyzeSpendingCapacity(domain: string, targetMarket: string) {
    ...
    return Object.fromEntries(
      Object.entries(baseCapacity).map(([key, value]) => [
        key,
        Math.round((value as number) * multiplier),
      ])
    )
  }
  ...
  private analyzePaymentCapabilities(domain: string) {
    const capabilityMap = {
      'health-fitness': {
        ...
        oneTimeMax: 500,
        annualMax: 1200,
        preferredFreq: 'monthly',
        budgetCycle: 'personal',
      },
      business: {
        monthlyMax: 2000,
        oneTimeMax: 10000,
        annualMax: 20000,
        preferredFreq: 'annual',
        budgetCycle: 'fiscal',
      },
      technology: {
        monthlyMax: 500,
      ...
      education: {
        ...
        oneTimeMax: 2000,
        annualMax: 3000,
        preferredFreq: 'course',
        budgetCycle: 'academic',
      },
    }
    return (
  ...
  private identifyAffordabilityBarriers(domain: string): string[] {
    const barrierMap = {
      'health-fitness': [
        'Limited wellness budget',
        'Competing fitness expenses',
        'Seasonal spending',
  ...
  private getCompetitorPrices(domain: string, category: string) {
    const pricesMap = {
      'health-fitness': [
        {
          competitor: 'MyFitnessPal Premium',
          price: 19.99,
          model: 'monthly',
          value: 'Nutrition tracking + workouts',
        },
        {
          competitor: 'Nike Training Club',
          price: 0,
          model: 'freemium',
          value: 'Basic workouts, premium coaching',
        },
        {
          competitor: 'Peloton Digital',
          price: 39.99,
          model: 'monthly',
          value: 'Live and on-demand classes',
        },
        {
          competitor: 'Fitbit Premium',
          price: 9.99,
          model: 'monthly',
          value: 'Advanced analytics + guidance',
        },
      ],
      business: [
        {
          ...
          price: 45,
          model: 'monthly',
          value: 'CRM + basic automation',
        },
        {
          competitor: 'Salesforce Essentials',
          price: 25,
          model: 'monthly',
          value: 'CRM for small business',
        },
        {
          competitor: 'Monday.com Basic',
          price: 8,
          model: 'monthly',
          value: 'Project management + collaboration',
        },
        {
          competitor: 'Zapier Starter',
          price: 19.99,
          model: 'monthly',
      ...
      technology: [
        ...
        {
          competitor: 'New Relic Standard',
          price: 99,
          model: 'monthly',
          value: 'Full stack monitoring',
        },
        {
          competitor: 'JetBrains IntelliJ',
          price: 16.9,
          model: 'monthly',
          value: 'IDE with advanced features',
        },
      ],
    }
    return (
  ...
  private identifyValueGaps(domain: string): string[] {
    const gapsMap = {
      'health-fitness': [
        'Mid-tier personalization',
        'Affordable premium features',
        'Family plan options',
      ],
      business: [
        'SMB-focused enterprise features',
  ...
  private identifyPositioningOpportunities(domain: string): string[] {
    const opportunitiesMap = {
      'health-fitness': [
        'AI-powered personalization leader',
        'Outcome-guaranteed fitness',
        'Integrated lifestyle platform',
      ],
      business: [
      ...
      technology: [
        ...
        'Zero-configuration leader',
        'Intelligence-first tools',
      ],
      finance: [
        'Transparency-first financial services',
        'Education-focused investing',
        'Ethical financial platform',
      ],
      education: [
        'Outcome-focused learning',
        'Real-world application leader',
        'Career acceleration platform',
      ],
      general: [
        'User-experience leader',
        'Value transparency champion',
        'Outcome-focused provider',
      ],
    }
    return opportunitiesMap[domain] || opportunitiesMap['general']
  }

  private synthesizeWillingnessToPay(
    marketPricing: any,
    valuePerception: any,
    affordability: any,
    ...
    const recommendedModel = Object.entries(modelPreferences).sort(
      ([, a], [, b]) => (b as number) - (a as number)
    )[0][0] as 'subscription' | 'one-time' | 'freemium' | 'usage-based' | 'hybrid'
    ...
    const confidence = Math.min(
      ...
      (sensitivity.valueOriented +
        ...
        (affordability.paymentCapabilities.monthlyMax > basePrice ? 80 : 60)) /
        3
    )
    ...
    return {
      ...
      valuePerspective: {
        ...
        costOfProblem,
        competitiveContext: competitivePricing.competitorPrices.map(
          (c: any) => `${c.competitor}: $${c.price}/${c.model}`
        ),
        ...
        segmentVariations: [
          {
            ...
            sensitivity: 'high',
            willingnessToPay: Math.round(minPrice),
          },
          {
            segment: 'Value-oriented',
            sensitivity: 'medium',
            willingnessToPay: Math.round((minPrice + maxPrice) / 2),
          },
  ...
  private generateDemandCurve(avgPrice: number, elasticity: string) {
    ...
    if (elasticity === 'high') {
      ...
      return basePoints.map(p => ({
        ...p,
        demandPercentage: Math.min(95, p.demandPercentage + 10),
      }))
    }
    return basePoints
  }
