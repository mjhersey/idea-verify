...
export class RequestValidator {
  validateCreateEvaluationRequest(data: any): CreateEvaluationRequest {
    ...
    if (!businessIdeaTitle || typeof businessIdeaTitle !== 'string') {
      throw new ValidationError('businessIdeaTitle is required and must be a string', 'businessIdeaTitle');
    }

    if (!businessIdeaDescription || typeof businessIdeaDescription !== 'string') {
      throw new ValidationError('businessIdeaDescription is required and must be a string', 'businessIdeaDescription');
    }
    ...
    if (businessIdeaTitle.length > 200) {
      throw new ValidationError('businessIdeaTitle must be 200 characters or less', 'businessIdeaTitle');
    }

    if (businessIdeaDescription.length > 2000) {
      throw new ValidationError('businessIdeaDescription must be 2000 characters or less', 'businessIdeaDescription');
    }
    ...
    if (agentTypes !== undefined) {
      ...
      const validAgentTypes: AgentType[] = ['market-research', 'competitive-analysis', 'customer-research', 'technical-feasibility', 'financial-analysis'];
      
      for (const agentType of agentTypes) {
        if (!validAgentTypes.includes(agentType)) {
          throw new ValidationError(`Invalid agent type: ${agentType}. Valid types are: ${validAgentTypes.join(', ')}`, 'agentTypes');
        }
      }
    ...
    if (priority !== undefined) {
      ...
      if (!validPriorities.includes(priority)) {
        throw new ValidationError(`Invalid priority: ${priority}. Valid priorities are: ${validPriorities.join(', ')}`, 'priority');
      }
  ...
  validateEvaluationQueryParams(query: any): EvaluationQueryParams {
    ...
    if (query.status !== undefined) {
      ...
      if (!validStatuses.includes(query.status)) {
        throw new ValidationError(`Invalid status: ${query.status}. Valid statuses are: ${validStatuses.join(', ')}`, 'status');
      }
    ...
    if (query.sortBy !== undefined) {
      ...
      if (!validSortFields.includes(query.sortBy)) {
        throw new ValidationError(`Invalid sortBy field: ${query.sortBy}. Valid fields are: ${validSortFields.join(', ')}`, 'sortBy');
      }
    ...
    if (query.sortOrder !== undefined) {
      ...
      if (!validSortOrders.includes(query.sortOrder)) {
        throw new ValidationError(`Invalid sortOrder: ${query.sortOrder}. Valid orders are: ${validSortOrders.join(', ')}`, 'sortOrder');
      }
  ...
  validateAgentQueryParams(query: any): AgentQueryParams {
    ...
    if (query.agentType !== undefined) {
      ...
      if (!validAgentTypes.includes(query.agentType)) {
        throw new ValidationError(`Invalid agent type: ${query.agentType}. Valid types are: ${validAgentTypes.join(', ')}`, 'agentType');
      }
