...
export class WebSocketServer extends EventEmitter {
  ...
  async start(server?: Server): Promise<void> {
    ...
    try {
      ...
      this.isStarted = true
      console.log(
        `[WebSocketServer] WebSocket server started on port ${this.config.port}${this.config.path}`
      )
      this.emit('started')
    } catch (error) {
  ...
  getClientsByEvaluationId(evaluationId: string): WebSocketClient[] {
    return Array.from(this.clients.values()).filter(
      client =>
        ...
        client.subscriptions.has(`evaluation:${evaluationId}`)
    )
  }
  ...
  broadcastToUser(userId: string, event: WebSocketEvent): void {
    ...
    console.log(
      `[WebSocketServer] Sent ${event.type} to ${clients.length} clients for user ${userId}`
    )
  }
  ...
  broadcastToEvaluation(evaluationId: string, event: WebSocketEvent): void {
    ...
    console.log(
      `[WebSocketServer] Sent ${event.type} to ${clients.length} clients for evaluation ${evaluationId}`
    )
  }
  ...
  private handleClientMessage(clientId: string, data: string): void {
    ...
    try {
      ...
      switch (message.type) {
        ...
        default:
          console.warn(
            `[WebSocketServer] Unknown message type from client ${clientId}:`,
            message.type
          )
      }
    } catch (error) {
  ...
  private handleClientDisconnect(clientId: string, code: number, reason: Buffer): void {
    this.removeClient(clientId)
    console.log(
      `[WebSocketServer] Client ${clientId} disconnected (code: ${code}, reason: ${reason.toString()})`
    )
  }
  ...
  private handleSubscription(client: WebSocketClient, data: any): void {
    ...
    console.log(
      `[WebSocketServer] Client ${client.id} subscribed to:`,
