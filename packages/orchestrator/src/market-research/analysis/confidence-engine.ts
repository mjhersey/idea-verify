...
export class ConfidenceEngine {
  ...
  private assessDataQuality(
    ...
    if (trends && trends.length > 0) {
      ...
      const avgTrendConfidence =
        trends.reduce((sum, trend) => sum + (trend.confidence || 0), 0) / trends.length
  ...
  private assessDataRecency(
    ...
    if (avgAge > 365)
      recencyScore = 30 // Over 1 year old
    else if (avgAge > 180)
      recencyScore = 50 // Over 6 months old
    else if (avgAge > 90)
      recencyScore = 70 // Over 3 months old
    else if (avgAge > 30)
      recencyScore = 85 // Over 1 month old
  ...
  private assessCrossValidation(
    ...
    if (uniqueSourceTypes.size >= 3)
      validationScore += 15 // Multiple source types
  ...
  private analyzeConfidenceFactors(
    ...
    if (components.dataQuality < 70) {
      recommendations.push(
        'Seek additional high-quality data sources to improve analysis reliability'
      )
    }

    if (components.sourceReliability < 70) {
      recommendations.push(
        'Prioritize authoritative sources such as government data and established research firms'
      )
    }
  ...
  private getImprovementRecommendation(component: string): string {
    const recommendations: Record<string, string> = {
      ...
      sourceReliability:
        'Seek out authoritative industry reports and government statistical sources',
      dataRecency: 'Update market analysis with current year data and real-time trend monitoring',
      methodologyRobustness:
        'Employ multiple calculation methodologies and document assumptions thoroughly',
      crossValidation: 'Cross-reference findings across multiple independent data sources',
    }
    return (
      recommendations[component] ||
      `Improve ${component} through additional research and validation`
    )
  }
  ...
  private calculateRecencyFactor(recencyDate: Date): number {
    ...
    if (ageInDays <= 365) return 0.7 // Significant reduction
    return 0.5 // Old data penalty
  }
}
