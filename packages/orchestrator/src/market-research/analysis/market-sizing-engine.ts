...
export class MarketSizingEngine {
  ...
  async calculateMarketSize(request: MarketSizingRequest): Promise<MarketSizeData> {
    ...
    const methodologyPromises = (request.preferredMethodologies || ['top-down', 'bottom-up', 'value-theory'])
  ...
  private async calculateTopDown(request: MarketSizingRequest): Promise<{
    ...
    const geographicFactor = this.calculateGeographicFactor(request.businessIdea.geography || ['US']);
  ...
  private async calculateGrowthRate(
    ...
    return {
      ...
      quarterly: [baseGrowthRate * 0.25, baseGrowthRate * 0.25, baseGrowthRate * 0.25, baseGrowthRate * 0.25],
      historical: [
        {
          period: '2022',
  ...
  private async generateProjections(
    ...
    const timeHorizon = 5;
    const scenarios = {
      conservative: this.createScenario('conservative', tam, sam, som, growthRate.annual * 0.7, 0.3),
  ...
  private calculateGeographicFactor(geography: string[]): number {
    ...
    return geography.reduce((factor, geo) => 
      factor + (geographicFactors[geo] || 0.1), 0
    );
  }
