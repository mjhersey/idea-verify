...
export class OpportunityScoringEngine {
  private readonly weights = {
    ...
    growth: 0.2, // 20% - Growth potential
    competition: 0.2, // 20% - Competitive intensity (inverse)
    trends: 0.15, // 15% - Market trends favorability
    barriers: 0.1, // 10% - Entry barriers (inverse)
    timing: 0.1, // 10% - Market timing
  }
  ...
  private scoreMarketSize(marketSize: MarketSizeData): number {
    ...
    let tamScore = 0
    if (tamValue >= 100000000000)
      tamScore = 100 // $100B+
    else if (tamValue >= 10000000000)
      tamScore = 90 // $10B+
    else if (tamValue >= 1000000000)
      tamScore = 80 // $1B+
    else if (tamValue >= 100000000)
      tamScore = 70 // $100M+
    else if (tamValue >= 10000000)
      tamScore = 60 // $10M+
    else if (tamValue >= 1000000)
      tamScore = 40 // $1M+
    else tamScore = 20
    ...
    if (samRatio < 0.05)
      tamScore *= 0.8 // Less than 5% addressable
    ...
    const avgConfidence =
      (marketSize.tam.confidence + marketSize.sam.confidence + marketSize.som.confidence) / 3
  ...
  private scoreGrowthPotential(growthRate: any): number {
    ...
    let growthScore = 0
    if (annualGrowth >= 20)
      growthScore = 100 // 20%+ growth
    else if (annualGrowth >= 15)
      growthScore = 90 // 15%+ growth
    else if (annualGrowth >= 10)
      growthScore = 80 // 10%+ growth
    else if (annualGrowth >= 7)
      growthScore = 70 // 7%+ growth
    else if (annualGrowth >= 5)
      growthScore = 60 // 5%+ growth
    else if (annualGrowth >= 3)
      growthScore = 50 // 3%+ growth
    else if (annualGrowth >= 0)
      growthScore = 30 // Positive growth
  ...
  private scoreCompetitiveEnvironment(competitive: CompetitiveLandscape): number {
    ...
    if (competitive.keyPlayers && competitive.keyPlayers.length > 0) {
      ...
      if (dominantPlayers.length >= 2)
        competitiveScore *= 0.8 // Multiple dominant players
  ...
  private scoreMarketTiming(request: MarketSizingRequest, trends: MarketTrend[]): number {
    ...
    if (trends && trends.length > 0) {
      const emergingTrends = trends.filter(
        t =>
          (t.impact === 'positive' && t.magnitude === 'high' && t.timeframe.includes('2024')) ||
          t.timeframe.includes('2025')
      )

      if (emergingTrends.length >= 2)
        timingScore = 90 // Multiple positive trends emerging
      else if (emergingTrends.length === 1) timingScore = 80 // One strong trend

      // Check for declining trends that might affect timing
      const decliningTrends = trends.filter(t => t.impact === 'negative' && t.magnitude === 'high')

      if (decliningTrends.length >= 2)
        timingScore = 40 // Multiple negative trends
    ...
    const categoryTiming =
      categoryTimingAdjustments[request.businessIdea.category || 'technology'] || 70
  ...
  private generateRecommendations(components: any, overall: number): string[] {
    ...
    if (components.growth < 50) {
      recommendations.push(
        'Explore strategies to accelerate market growth or find faster-growing segments'
      )
    }

    if (components.competition < 40) {
      recommendations.push(
        'Develop strong differentiation strategy and consider blue ocean opportunities'
      )
    }

    if (components.trends < 50) {
      recommendations.push(
        'Monitor market trends closely and adapt strategy to align with positive trends'
      )
    }

    if (components.barriers > 80) {
      recommendations.push(
        'Leverage low entry barriers for rapid market entry before competition increases'
      )
    } else if (components.barriers < 40) {
      recommendations.push(
        'Develop sustainable competitive advantages to overcome high entry barriers'
      )
    }

    if (components.timing > 80) {
      recommendations.push(
        'Excellent timing - prioritize speed to market to capitalize on current conditions'
      )
    } else if (components.timing < 50) {
      recommendations.push(
        'Consider timing strategy - may benefit from delayed entry or market education'
      )
    }
