...
export class DataSourceManager {
  ...
  async collectMarketData(request: MarketSizingRequest): Promise<{
    ...
    const averageQuality = totalSources > 0 
      ? allResults.reduce((sum, r) => sum + r.quality, 0) / totalSources 
      : 0;
  ...
  private async collectFromSources(
    ...
    const scrapingRequests = sources.map(source => this.createScrapingRequest(source, sourceType, request));
  ...
  private getSourceCredibility(source: string): number {
    const credibilityMap: { [key: string]: number } = {
      // High credibility
      'gartner': 95, 'forrester': 95, 'mckinsey': 95,
      'census': 98, 'bls': 96, 'education-dept': 94,
      'sec-filings': 97, 'reuters': 90, 'bloomberg': 90,
      
      // Medium credibility
      'techcrunch': 80, 'yahoo-finance': 75,
      'industry-associations': 85, 'government-reports': 90,
      'analyst-reports': 80, 'financial-news': 82,
      
      // Lower credibility
      'general-news': 70, 'industry-blogs': 65,
      'social-scraping': 60, 'trend-aggregators': 68,
      'state-agencies': 78, 'international-orgs': 85
    };
