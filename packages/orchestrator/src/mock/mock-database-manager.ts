...
export class MockDatabaseManager extends EventEmitter implements IDatabaseManager {
  ...
  async query<T = any>(text: string, params?: any[]): Promise<T[]> {
    ...
    this.queryCount++
    console.log(
      `[MockDatabaseManager] Executed query { text: '${text.slice(0, 50)}${text.length > 50 ? '...' : ''}', duration: '${Math.random() * 100}ms', rows: ${Math.floor(Math.random() * 10)} }`
    )
    ...
    const businessIdeas = [
      {
        ...
        description:
          'A mobile app that uses AI to provide personalized fitness recommendations and workout plans based on user goals, progress, and preferences.',
        status: 'submitted',
        created_at: new Date(),
        updated_at: new Date(),
      },
      {
        ...
        description:
          'A platform that connects e-commerce businesses with sustainable packaging suppliers, reducing environmental impact while maintaining cost-effectiveness.',
        status: 'submitted',
        created_at: new Date(),
        updated_at: new Date(),
      },
        ...
        if (setClause.includes('status')) {
          record.status =
            params.find(
              p =>
                typeof p === 'string' &&
                ['pending', 'analyzing', 'completed', 'failed', 'running'].includes(p)
            ) || record.status
        }

        if (setClause.includes('score')) {
          record.score =
            params.find(p => typeof p === 'number' && p >= 0 && p <= 100) || record.score
        }
