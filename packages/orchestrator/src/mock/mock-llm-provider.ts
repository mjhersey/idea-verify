...
export class MockLLMProvider implements LLMProvider {
  ...
  private extractKeyFromPrompt(prompt: string): string {
    ...
    if (
      lowerPrompt.includes('technical feasibility') ||
      lowerPrompt.includes('technical analysis')
    ) {
  ...
  private generateMockAudience(businessTitle: string): any {
    return {
      ...
      painPoints: ['Time constraints', 'Cost concerns', 'Complexity issues', 'Reliability needs'],
    }
  }
