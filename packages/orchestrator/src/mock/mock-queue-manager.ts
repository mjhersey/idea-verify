...
export class MockQueueManager extends EventEmitter implements IQueueManager {
  ...
  async addAgentJob(data: AgentTaskJobData, options: JobOptions = {}): Promise<string> {
    const job = this.createJob(data, options)
  ...
  private findJob(jobId: string): MockJob | undefined {
    return this.evaluationJobs.get(jobId) || this.agentJobs.get(jobId) || this.resultJobs.get(jobId)
  }
