...
export class IntegrationTestSuite {
  ...
  async runAllTests(): Promise<IntegrationTestResult[]> {
    ...
    console.log(
      `Mode: ${process.env.USE_MOCK_SERVICES === 'true' ? 'Mock Services' : 'Real Services'}`
    )
    ...
    for (const service of services) {
      ...
      try {
        ...
        if (process.env.USE_MOCK_SERVICES === 'true') {
          ...
          credentials = await this.secretsManager.getCredentials(
            service as keyof ExternalServiceCredentials
          )
        } else {
        ...
        switch (service) {
          case 'openai':
            ...
            break
          case 'anthropic':
            validationResult = await this.validator.validateAnthropic(
              credentials as AnthropicCredentials
            )
            break
          case 'aws':
    ...
    for (const service of services) {
      ...
      try {
        ...
        const hasRecommendations =
          (status.fallbackRecommendations?.recommendations?.length || 0) > 0
  ...
  private getMockCredentials(
    service: string
  ): OpenAICredentials | AnthropicCredentials | AWSCredentials | Record<string, never> {
    switch (service) {
      case 'openai':
