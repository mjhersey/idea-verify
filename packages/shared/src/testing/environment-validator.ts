...
export class EnvironmentValidator {
  ...
  private async checkApiHealth(): Promise<ValidationCheck> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const hasUnhealthyServices = Object.values(healthData.services || {})
          .some((service: any) => service.status !== 'healthy');

        return {
          name: 'API Service Health',
          category: 'services',
          status: hasUnhealthyServices ? 'warning' : 'pass',
          message: hasUnhealthyServices ? 'Some services are unhealthy' : 'All API services are healthy',
          details: healthData,
          duration,
        };
      } else {
  ...
  private async checkAuthenticationEndpoints(): Promise<ValidationCheck> {
    ...
    try {
      ...
      return {
        ...
        message: allResponding ? 'Authentication endpoints are responding' : 'Some auth endpoints are not responding correctly',
  ...
  private async checkBusinessEndpoints(): Promise<ValidationCheck> {
    ...
    try {
      ...
      return {
        ...
        message: allResponding ? 'Business endpoints require authentication' : 'Business endpoints may have issues',
        details: { endpoints: results },
        duration,
      };
    } catch (error) {
  ...
  private async checkEnvironmentVariables(): Promise<ValidationCheck> {
    ...
    return {
      ...
      message: missing.length === 0 ? 'All required environment variables are set' : `Missing variables: ${missing.join(', ')}`,
  ...
  private async checkCorsConfiguration(): Promise<ValidationCheck> {
    ...
    try {
      ...
      return {
        ...
        message: hasCorsHeaders ? 'CORS headers are configured' : 'CORS headers may not be configured',
        details: { corsHeaders },
        duration,
      };
    } catch (error) {
  ...
  private async checkRateLimiting(): Promise<ValidationCheck> {
    ...
    try {
      ...
      const status = (rateLimitedResponses > 0 || hasRateLimitHeaders) ? 'pass' : 
                    (this.environment === 'prod' ? 'warning' : 'pass');
      ...
      return {
        ...
        message: rateLimitedResponses > 0 ? 
          ...
          hasRateLimitHeaders ? 'Rate limiting headers present' : 'No rate limiting detected',
