...
export class MockAnthropicService {
   ...
   */
  private handleMessages(req: any, res: any): void {
    let body = ''
    ...
    req.on('end', () => {
      try {
        ...
        res.setHeader(
          ...
          new Date(this.lastReset + 60000).toISOString()
        )
   ...
   */
  private generateMockMessage(request: any): AnthropicMessage {
    ...
    const inputTokens = this.estimateTokens(messages.map((m: any) => m.content).join(' '))
