...
export class HealthMonitor {
   ...
   */
  private startMonitoring(serviceName: string, healthCheckFn: () => Promise<Record<string, unknown>>): void {
  ...
  private monitoredServices: Map<string, { healthCheckFn: () => Promise<Record<string, unknown>> }> = new Map();
   ...
   */
  private async performHealthCheck(serviceName: string, healthCheckFn: () => Promise<Record<string, unknown>>): Promise<HealthStatus> {
    const config = this.configs.get(serviceName)!;
