...
export class UserRepository {
  ...
  async findByIdWithBusinessIdeas(id: string): Promise<UserWithBusinessIdeas | null> {
    try {
      ...
      throw new Error(
        `Failed to find user with business ideas: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  async isEmailTaken(email: string, excludeUserId?: string): Promise<boolean> {
    try {
      ...
      throw new Error(
        `Failed to check email availability: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
