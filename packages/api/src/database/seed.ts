...
const seedBusinessIdeas: (Omit<CreateBusinessIdeaInput, 'user_id'> & { userEmail: string })[] = [
  ...
  },
  {
    ...
    description:
      'An IoT-based platform that automatically optimizes home energy consumption by learning usage patterns, weather forecasts, and utility pricing. The system controls smart appliances, HVAC systems, and solar panels to minimize energy costs while maintaining comfort.',
  ...
  },
  {
    ...
    description:
      'A comprehensive VR fitness ecosystem offering immersive workout experiences from home. Users can participate in virtual fitness classes, compete with friends in gamified exercises, and track their progress with AI-powered form analysis and personalized coaching.',
  ...
  },
  {
    ...
    description:
      'An online marketplace that connects consumers with sustainable fashion brands and promotes circular economy practices. Features include authenticity verification, sustainability scoring, clothing rental options, and a take-back program for end-of-life garments.',
  ...
  },
  {
    ...
    description:
      'A scalable indoor farming solution using hydroponics, LED grow lights, and AI-controlled environment management. Target markets include restaurants, grocery stores, and urban communities seeking fresh, locally-grown produce year-round.',
    status: 'completed' as BusinessIdeaStatus,
  },
  {
    userEmail: 'bob.wilson@example.com',
    ...
    description:
      'A decentralized identity management system that gives users complete control over their personal data. Businesses can verify customer identities without storing sensitive information, reducing data breach risks and improving privacy compliance.',
    status: 'evaluating' as BusinessIdeaStatus,
  },
]
      ...
      if (
        ...
        const evaluation = await prisma.evaluation.create({
          data: {
            ...
            status:
              idea.status === 'completed'
                ? ('completed' as EvaluationStatus)
                : idea.status === 'evaluating'
                  ? ('analyzing' as EvaluationStatus)
      ...
      for (const agentType of agentTypes) {
        ...
        const agentResult = await prisma.agentResult.create({
          data: {
            ...
            status: isCompleted
              ? ('completed' as AgentResultStatus)
              : isRunning
                ? ('running' as AgentResultStatus)
                : ('pending' as AgentResultStatus),
            input_data: {
...
export async function validateSeedData(): Promise<{
  ...
  try {
    ...
    return {
      valid: false,
      issues: [
        `Seed data validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      ],
    }
  }
}
