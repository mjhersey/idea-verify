    ...
    try {
      ...
      if (status.appliedMigrations.length > 0) {
        ...
        status.appliedMigrations.slice(0, 5).forEach(migration => {
          const status = migration.rolled_back_at ? '❌ ROLLED BACK' : 
                        migration.finished_at ? '✅ COMPLETED' : '⏳ PENDING';
          console.log(`  ${status} ${migration.migration_name} (${migration.started_at.toISOString()})`);
        });
      }
