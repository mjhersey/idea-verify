...
router.get('/status', async (req, res) => {
  try {
    const [health, info, connectionLoad, migrationStatus, migrationValidation] = await Promise.all([
      ...
      getDatabaseInfo(),
      monitorConnectionLoad(),
      getMigrationStatus().catch(() => ({ appliedMigrations: [], pendingMigrations: [], hasUnappliedMigrations: false })),
...
router.get('/metrics', async (req, res) => {
  try {
    const [info, connectionLoad] = await Promise.all([
      getDatabaseInfo(),
      monitorConnectionLoad()
    ]);
    ...
    res.json({
      connection_pool: {
        ...
        utilization_percent: info.connectionStats ? 
          ((info.connectionStats.total / info.connectionStats.maxConnections) * 100).toFixed(2) : '0'
      },
      health: {
        healthy: connectionLoad.healthy,
        warnings: connectionLoad.warnings
