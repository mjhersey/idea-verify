...
export class WebSocketRateLimiter {
  ...
  canConnect(userId: string, socketId: string): { allowed: boolean; reason?: string } {
    ...
    if (userConnectionCount >= this.config.maxConnectionsPerUser) {
      return { allowed: false, reason: `User connection limit exceeded (${userConnectionCount}/${this.config.maxConnectionsPerUser})` };
    }
  ...
  addConnection(userId: string, socket: Socket): void {
    ...
    console.log(`[RateLimit] Connection registered: user=${userId}, socket=${socketId}, total=${this.connections.size}`);
  }
  ...
  removeConnection(socketId: string): void {
    ...
    console.log(`[RateLimit] Connection removed: user=${userId}, socket=${socketId}, total=${this.connections.size}`);
  }
  ...
  canSendMessage(socketId: string): { allowed: boolean; reason?: string; waitMs?: number } {
    ...
    const messagesInBurstWindow = recentMessages.filter(timestamp => timestamp > burstWindowStart).length;
  ...
  private recordViolation(userId: string, socketId: string, violationType: string): void {
    ...
    console.warn(`[RateLimit] Violation recorded: user=${userId}, socket=${socketId}, type=${violationType}, total=${metrics.violations}`);
  ...
  private banUser(userId: string, socketId: string): void {
    ...
    console.warn(`[RateLimit] User banned: user=${userId}, duration=${this.config.banDurationMs}ms, expires=${new Date(metrics.banExpiresAt).toISOString()}`);
  }
  ...
  getMetrics(): {
    ...
    return {
      ...
      userCount,
      bannedUsers,
      avgMessagesPerConnection: totalConnections > 0 ? Math.round(totalMessages / totalConnections) : 0,
      totalViolations
    };
  }
  ...
  cleanup(): void {
    ...
    if (cleanedConnections > 0 || clearedBans > 0) {
      console.log(`[RateLimit] Cleanup completed: cleared ${clearedBans} bans, cleaned ${cleanedConnections} violation records`);
    }
  }
