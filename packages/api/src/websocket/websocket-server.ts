  ...
  private async validateEvaluationAccess(userId: string, evaluationId: string): Promise<boolean> {
    try {
      ...
      if (!userId || !evaluationId) {
        console.warn(`[WebSocket] Invalid access validation - userId: ${userId}, evaluationId: ${evaluationId}`);
        return false;
      }
      ...
      try {
        ...
        if (evaluationRecord.user_id !== userId) {
          console.warn(`[WebSocket] Access denied: User ${userId} does not own evaluation ${evaluationId}`);
          return false;
        }
        ...
        if (!validStatuses.includes(evaluationRecord.status)) {
          console.warn(`[WebSocket] Evaluation ${evaluationId} has invalid status: ${evaluationRecord.status}`);
          return false;
        }
        ...
        if (currentConnections >= maxConnections) {
          console.warn(`[WebSocket] Rate limit exceeded: User ${userId} has ${currentConnections}/${maxConnections} active connections`);
          return false;
        }
        ...
        console.log(`[WebSocket] Database validation successful for user ${userId} to evaluation ${evaluationId}`);
        return true;
        
      } catch (dbError) {
        // Graceful fallback when database is unavailable
        console.warn(`[WebSocket] Database unavailable, using fallback validation:`, dbError?.message || dbError);
        ...
        console.log(`[WebSocket] Fallback validation successful for user ${userId} to evaluation ${evaluationId}`);
        return true;
      }
