...
export class ProgressCalculator {
  ...
  private readonly defaultWeights: Map<string, AgentWeight> = new Map([
    ...
    ['technical-feasibility', { agentType: 'technical-feasibility', weight: 0.30, estimatedDurationMs: 50000 }],
    ['competitive-analysis', { agentType: 'competitive-analysis', weight: 0.25, estimatedDurationMs: 40000 }],
    ['customer-research', { agentType: 'customer-research', weight: 0.20, estimatedDurationMs: 35000 }]
  ]);
  ...
  initializeEvaluation(
    ...
    console.log(`[ProgressCalculator] Initialized evaluation ${evaluationId} with ${agentTypes.length} agents`);
  }
  ...
  updateAgentProgress(
    ...
    if (!agentProgress) {
      console.warn(`[ProgressCalculator] Agent ${agentType} not found in evaluation ${evaluationId}`);
    ...
    console.log(`[ProgressCalculator] Updated ${agentType} to ${status} (${progressPercentage}%), overall: ${evaluation.overallProgress}%`);
  ...
  getEvaluationProgress(evaluationId: string): {
    ...
    evaluation.agentProgresses.forEach((progress, agentType) => {
      ...
      if (progress.status === 'running' || progress.status === 'initializing' || progress.status === 'retrying') {
  ...
  getEvaluationMetrics(evaluationId: string): {
    ...
    const averageAgentTime = completedAgentTimes.length > 0 
    ...
    const efficiency = completedAgentTimes.length > 0 
