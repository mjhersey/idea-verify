...
describe('BusinessIdeaService', () => {
  ...
  describe('createIdea', () => {
    const validInput = {
      ...
      description:
        'This is a detailed description of my business idea that is over 50 characters long and contains meaningful content about the business.',
    }
    ...
    it('should generate title from description when title not provided', async () => {
      const inputWithoutTitle = {
        userId: 'user-123',
        description:
          'This is a very long description that should be used to generate a title automatically when no title is provided by the user.',
      }
    ...
    it('should sanitize HTML tags from input', async () => {
      const inputWithHTML = {
        ...
        description:
          'This is a description with <b>HTML tags</b> and <script>malicious code</script> that should be sanitized to prevent XSS attacks.',
      }
    ...
    it('should remove javascript: protocols', async () => {
      const inputWithJavaScript = {
        userId: 'user-123',
        description:
          'This description contains javascript:alert("xss") which should be removed for security reasons and this makes it long enough.',
      }
    ...
    it('should reject content with insufficient words', async () => {
      ...
      await expect(service.createIdea(shortInput)).rejects.toThrow(
    ...
    it('should reject content with excessive capitalization', async () => {
      const capsInput = {
        userId: 'user-123',
        description:
          'THIS IS A VERY LONG DESCRIPTION WITH TOO MANY CAPITAL LETTERS WHICH SHOULD BE REJECTED FOR BEING TOO AGGRESSIVE AND SPAM-LIKE',
      }

      await expect(service.createIdea(capsInput)).rejects.toThrow(
    ...
    it('should normalize whitespace in input', async () => {
      const inputWithExtraSpaces = {
        userId: 'user-123',
        description:
          'This   description   has    multiple   spaces   that   should   be   normalized   to   single   spaces   for   better   formatting.',
      }
    ...
    it('should trim input content', async () => {
      const inputWithSpaces = {
        ...
        description:
          '   This description has leading and trailing spaces that should be trimmed for consistency.   ',
      }
  ...
  describe('generateTitleFromDescription', () => {
    ...
    it('should truncate long descriptions and add ellipsis', () => {
      const longDescription =
        'This is a very long business idea description that exceeds fifty characters and should be truncated properly'
    ...
    it('should break at word boundaries', () => {
      const description =
        'This is a moderately long business idea description that should break at word boundaries'
