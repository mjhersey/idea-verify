...
describe.skipIf(!process.env.DATABASE_URL || process.env.NODE_ENV === 'ci')(
  ...
  () => {
    ...
    describe('POST /api/ideas', () => {
      const validIdea = {
        ...
        description:
          'This is a comprehensive business idea description that contains enough characters to meet the minimum requirement of 50 characters and provides meaningful content about the business concept.',
      }
      ...
      it('should create idea without title', async () => {
        const ideaWithoutTitle = {
          description:
            'This is a business idea description without a title that should automatically generate one from the description content provided here.',
        }
      ...
      it('should require authentication', async () => {
        const response = await request(app).post('/api/ideas').send(validIdea).expect(401)
      ...
      it('should sanitize HTML content', async () => {
        const maliciousIdea = {
          ...
          description:
            'This description contains <script>alert("xss")</script> malicious content and <b>HTML tags</b> that should be sanitized for security purposes.',
        }
      ...
      it('should log successful submissions', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Business idea submitted:'))
