...
class EvaluationPipelineTester {
  ...
  async testComplexEvaluationFlow(): Promise<void> {
    ...
    const businessIdea = {
      ...
      description:
        'An AI-powered business intelligence platform that analyzes market trends, competitor data, and consumer behavior to provide actionable insights for e-commerce businesses. Features include predictive analytics, automated reporting, real-time dashboards, and integration with major e-commerce platforms like Shopify, Amazon, and WooCommerce.',
      targetMarket:
        'e-commerce businesses, online retailers, digital marketing agencies, business analysts',
      revenue: 500000,
      costs: 200000,
    }
  ...
  async testMarketResearchAgent(): Promise<void> {
    ...
    expect(['small', 'medium', 'large', 'very large']).toContain(
      marketResearch.marketSize.toLowerCase()
    )

    if (marketResearch.competition) {
      expect(['low', 'moderate', 'high', 'very high']).toContain(
        marketResearch.competition.toLowerCase()
      )
    }
  ...
  async testFinancialAnalysisAgent(): Promise<void> {
    ...
    if (financialAnalysis.riskLevel) {
      expect(['low', 'medium', 'high', 'very high']).toContain(
        financialAnalysis.riskLevel.toLowerCase()
      )
    }
  ...
  async testEvaluationErrorHandling(): Promise<void> {
    ...
    if (invalidIdeaResponse.status === 201) {
      ...
      const evalResponse = await this.makeRequest(
        'POST',
        `/api/ideas/${createdInvalidIdea.id}/evaluate`
      )
  ...
  async testEvaluationCancellation(): Promise<void> {
    ...
    const cancelResponse = await this.makeRequest(
      'DELETE',
  ...
  async testResultDataQuality(): Promise<void> {
    const businessIdea = {
      ...
      description:
        'A comprehensive business idea to test the quality and completeness of evaluation results.',
...
describe('Evaluation Pipeline Tests', () => {
  ...
  describe('Basic Evaluation Flow', () => {
    test(
      'should complete simple evaluation successfully',
      async () => {
    ...
    test(
      'should handle complex evaluation flow',
      async () => {
  ...
  describe('Agent Integration', () => {
    test(
      'should integrate with market research agent',
      async () => {
    ...
    test(
      'should integrate with financial analysis agent',
      async () => {
  ...
  describe('Error Handling', () => {
    ...
    test(
      'should support evaluation cancellation',
      async () => {
  ...
  describe('Performance', () => {
    test(
      'should meet evaluation performance requirements',
      async () => {
  ...
  describe('Data Quality', () => {
    test(
      'should produce high-quality evaluation results',
      async () => {
