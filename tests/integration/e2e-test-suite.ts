...
class E2ETestSuite {
  ...
  async setup(): Promise<void> {
    ...
    if (!validationResult.valid) {
      ...
      throw new Error(`Environment validation failed: ${failedChecks.map(c => c.message).join(', ')}`);
    }
  ...
  private async makeRequest(method: string, path: string, body?: any, headers?: Record<string, string>): Promise<Response> {
  ...
  private async makeAuthenticatedRequest(method: string, path: string, body?: any): Promise<Response> {
    if (!this.testUser.token) {
  ...
  async testBusinessIdeaSubmission(): Promise<BusinessIdea> {
    const idea: BusinessIdea = {
      name: 'E2E Test Idea',
      description: 'A revolutionary app that helps people test their business ideas using AI validation.',
  ...
  async testEvaluationStatusPolling(evaluationId: string): Promise<void> {
    ...
    while (attempts < maxAttempts) {
      const response = await this.makeAuthenticatedRequest('GET', `/api/evaluations/${evaluationId}`);
      expect(response.status).toBe(200);
  ...
  async testEvaluationResultRetrieval(evaluationId: string): Promise<void> {
    const response = await this.makeAuthenticatedRequest('GET', `/api/evaluations/${evaluationId}/result`);
  ...
  async testConcurrentRequests(): Promise<void> {
    const promises = Array(10).fill(null).map(() => 
      this.makeAuthenticatedRequest('GET', '/api/user/profile')
    );
...
describe('E2E Tests - Deployed Environment', () => {
  ...
  describe('Business Logic Flow', () => {
    ...
    test('should complete evaluation', async () => {
