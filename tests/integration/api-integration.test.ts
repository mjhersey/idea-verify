...
import { getTestEnvironment, isFeatureEnabled, getTestUser, shouldSkipSlowTests } from '../config/test-environments.js';
...
class ApiIntegrationTester {
  ...
  async testBusinessIdeasEndpoints(): Promise<void> {
    ...
    const updateResponse = await this.makeRequest('PATCH', `/api/ideas/${this.context.ideaId}`, updateData);
    expect(updateResponse.status).toBe(200);
  ...
  async testEvaluationEndpoints(): Promise<void> {
    ...
    // Start evaluation
    const startResponse = await this.makeRequest('POST', `/api/ideas/${this.context.ideaId}/evaluate`);
    ...
    const statusResponse = await this.makeRequest('GET', `/api/evaluations/${this.context.evaluationId}`);
    ...
    const resultResponse = await this.makeRequest('GET', `/api/evaluations/${this.context.evaluationId}/result`);
  ...
  async testSecurityFeatures(): Promise<void> {
    ...
    logger.info('Security features test passed', { 
      corsHeaders: Object.keys(corsHeaders).filter(key => corsHeaders[key as keyof typeof corsHeaders]),
      securityHeaders: Object.keys(securityHeaders).filter(key => securityHeaders[key as keyof typeof securityHeaders]),
    });
  }
...
describe('API Integration Tests', () => {
  ...
  describe('User Management', () => {
    test('should handle user operations correctly', async () => {
  ...
  describe('Evaluations', () => {
    test('should handle evaluation lifecycle', async () => {
