...
describe('WebSocket Performance Tests', () => {
  ...
  describe('Connection Performance', () => {
    it(
      'should establish connections within acceptable time limits',
      async () => {
    ...
    it(
      'should handle connection spikes without degradation',
      async () => {
        ...
        batchResults.forEach(result => {
          console.log(
            `- Batch ${result.batchSize}: avg=${result.avgTime.toFixed(2)}ms, max=${result.maxTime.toFixed(2)}ms`
          )
        })
  ...
  describe('Message Throughput Performance', () => {
    it(
      'should maintain high message throughput under load',
      async () => {
        ...
        return new Promise<void>((resolve, reject) => {
          const timeout = setTimeout(() => {
            reject(
              new Error(
                `Throughput test timeout. Received ${receivedCount}/${messageCount} messages`
              )
            )
          }, 30000)
    ...
    it(
      ...
      async () => {
        ...
        return new Promise<void>((resolve, reject) => {
          const timeout = setTimeout(() => {
            reject(
              new Error(`Message loss detected. Sent: ${totalSent}, Received: ${totalReceived}`)
            )
          }, 30000)
          ...
          testSocket.on('agent:progress', (data: AgentProgressEvent & { messageId?: string }) => {
            ...
            if (totalReceived >= totalSent && totalSent === burstSize * burstCount) {
              ...
              console.log(
                `- Message loss rate: ${(((totalSent - totalReceived) / totalSent) * 100).toFixed(2)}%`
              )
  ...
  describe('Scalability Validation', () => {
    it(
      'should handle maximum concurrent connections',
      async () => {
        ...
        console.log(
          `Successfully handled ${maxConnections} concurrent connections with message distribution`
        )
      },
      LOAD_TEST_TIMEOUT
    )
